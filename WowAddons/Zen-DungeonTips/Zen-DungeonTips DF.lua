...
addon.acceptedDungeons = {
    ...
    -- TWW
    [293] = true, -- Grim Batol dungeon
    [2341] = true, -- Stonevault
    [2292] = true, -- Nerubar
    [2294] = true, -- Nerubar2
    [2293] = true, -- Nerubar maybes?
    [2295] = true, -- Nerubar maybes?
    [2296] = true, -- Nerubar maybes?
    [2357] = true, -- Ara'Kara City of Echoes
    [2359] = true, -- Dawnbreaker
    [2343] = true, -- City of threads
...
local function addLine(tooltip, tips, type, role, class)
	...
	if not found then
		...
		for i, tip in ipairs(tips) do
			...
			if ZenConfig[tip[1]] or tip[1] == "Legion" or tip[1] == "Dodge" or -- Show if tip type turned on, or if it's using an old Legion tag.
				(tip[1] == role and ZenConfig["RoleChoice"] == "Show my role only") or  -- Show if role matches the tip, and the user wants to see their role only.
				(roleList[tip[1]] and ZenConfig["RoleChoice"] == "Show all roles") or -- Show if role tip and user wants to see all.
				
				(tip[1] == class and ZenConfig["ClassChoice"] == "Show my class only") or
				(classList[tip[1]] and ZenConfig["ClassChoice"] == "Show all classes") then
...
local function addFrameLine(tooltip, tips, type, role, class)
	local found = false
	-- Check if we already added to this tooltip. This prevents writing the same thing to the tooltip multiple times.
	if not Zen_HeaderPanel:IsVisible() then addon:setEnabled() end
		...
		for i, tip in ipairs(tips) do
			...
			if ZenConfig[tip[1]] or tip[1] == "Legion" or tip[1] == "Dodge" or -- Show if tip type turned on, or if it's using an old Legion tag.
				...
				(classList[tip[1]] and ZenConfig["ClassChoice"] == "Show all classes") then
					...
					local tipBase = Zen_TipText:GetText() or ""
					
					if iconList[tip[1]] then -- Check if Icon exists
						--tooltip:AddLine((("|T%s:0|t"):format("Interface\\Icons\\"..iconList[tip[1]])..tip[2]),r,g,b)
						
						Zen_TipText:SetText(tipBase .. ((("|T%s:0|t"):format("Interface\\Icons\\"..iconList[tip[1]]).. "|cff" .. lineHex .. " " .. tip[2] .. "|r" .. "\n")))
						
					elseif tipsColors[tip[1]] then -- Check if color exists
						Zen_TipText:SetText(tipBase .. "|cff" .. lineHex .. " " .. tip[2] .. "|r" .. "\n")
						--tooltip:AddLine(tip[2],r,g,b)
					else -- There is no icon or color assigned to the category so a plain line will be added instead.
						Zen_TipText:SetText(tipBase .. " " .. tip[2] .. "\n")
						--tooltip:AddLine(tip[2])
					end
			end
		end
...
local function addLineNew(tooltip, data)
  ...
  if ZenConfig.ShowFrame == "Show in separate frame" and ZenConfig.TargetTrigger == "Show targeted mob" then return end -- Tiny Snippet to disable the tooltip hook if targeting is selected instead.
  if ZenConfig.ShowFrame == "Show in separate frame" and ZenConfig.TargetTrigger == "Show mouseover" and Zen_onBoss then return end -- Disable tooltip hook if player is using frame + Mouseover but is on boss
  ...
  local _, class, _ = UnitClass("player")
	...
	if data then
		...
		if tipsMap[id] then
			...
			Zen_TipText:SetText("")
			Zen_MobName:SetText(name)	

			if ZenConfig.ShowFrame == "Show in separate frame" then addFrameLine(Zen_TipPanel, tipsMap[id], "NPC ID:", role, class) 
...
function addon:getTarget(mobType)
  ...
  if guid then
	...
	if tipsMap[id]  then
		...
		Zen_TipText:SetText("")		
		Zen_MobName:SetText(name)
		addFrameLine(Zen_TipPanel, tipsMap[id], "NPC ID:", role, class)
