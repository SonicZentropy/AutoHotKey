...
local Type, Version = "ZekiliCustomEditor", 5
...
if not ZekiliCustomEditorInsertLink then
  local function ZekiliCustomEditorInsertLink(text)
    for i = 1, AceGUI:GetWidgetCount(Type) do
      local editbox = _G[("ZekiliCustomEditor%uEdit"):format(i)]
  ...
  -- upgradeable hook
  hooksecurefunc("ChatEdit_InsertLink", function(...) return ZekiliCustomEditorInsertLink(...) end)
end
...
local function GenerateDiagnosticTooltip( widget, event )
    ...
    local pack = rawget( Zekili.DB.profile.packs, packName )
