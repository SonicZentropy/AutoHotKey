...
Zekili.unitIDs = unitIDs
Zekili.npGUIDs = npGUIDs
Zekili.npUnits = npUnits


function Zekili:GetNameplateUnitForGUID( id )
    return npUnits[ id ]
end

function Zekili:GetGUIDForNameplateUnit( unit )
    return npGUIDs[ unit ]
end

function Zekili:GetUnitByGUID( id )
    for _, unit in ipairs( unitIDs ) do
        if UnitGUID( unit ) == id then return unit end
    end
end

function Zekili:GetUnitByName( name )
    for _, unit in ipairs( unitIDs ) do
...
do
    ...
    function Zekili:GetPetBasedTargetSpells()
        return petSpells[ myClass ]
    end

    function Zekili:CanUsePetBasedTargetDetection()
        return petSpells[ myClass ] ~= nil
    end

    function Zekili:HasPetBasedTargetSpell()
        return petSlot > 0
    end

    function Zekili:GetPetBasedTargetSpell()
        return petAction > 0 and petAction or nil
    end

    function Zekili:PetBasedTargetDetectionIsReady( skipRange )
    ...
    function Zekili:SetupPetBasedTargetDetection()
        petAction = 0
        petSlot = 0
    ...
    function Zekili:TargetIsNearPet( unit )
        return IsActionInRange( petSlot, unit )
    end

    function Zekili:DumpPetBasedTargetInfo()
...
do
    ...
    if not Zekili.IsDragonflight() then
...
do
    ...
    function ns.getNumberTargets( forceUpdate )
        ...
        spec = spec and rawget( Zekili.DB.profile.specs, spec )
        ...
        local checkPets = showNPs and spec and spec.petbased and Zekili:PetBasedTargetDetectionIsReady()
                    ...
                    if UnitExists( unit ) and not UnitIsDead( unit ) and UnitCanAttack( "player", unit ) and UnitInPhase( unit ) and UnitHealth( unit ) > 1 and ( not inGroup or not FriendCheck( unit ) ) and ( UnitIsPVP( "player" ) or not UnitIsPlayer( unit ) ) then
                        ...
                        if excluded then
                            ...
                            if not excluded and checkPets then
                                excluded = not Zekili:TargetIsNearPet( unit )
                        ...
                        if not excluded then
                            local rate, n = Zekili:GetTTD( unit )
                            count = count + 1
                            counted[ guid ] = true
        ...
        if not spec or spec.damage or not checkPets and not checkPlates then
            ...
            for guid, seen in pairs(db) do
                if counted[ guid ] == nil then
                    ...
                    local unit = Zekili:GetUnitByGUID( guid ) or UnitTokenFromGUID( guid )
        ...
        if count ~= lastCount or stationary ~= lastStationary then
            ...
            if Zekili:GetToggleState( "mode" ) == "reactive" then ZekiliDisplayAOE:UpdateAlpha() end
        end

        if details then
            Zekili.TargetDebug = details
        end
...
function Zekili:GetNumTargets( forceUpdate )
...
ns.reportTargets = function()
    for k, v in pairs(targets) do
        Zekili:Print("Saw " .. k .. " exactly " .. GetTime() - v .. " seconds ago.")
    end
end
...
function Zekili:HasMinionID(id)
...
do
    ns.Audit = function( special )
        ...
        if ZekiliEngine:IsThreadActive() then
            return
        end
        ...
        if not special and not Zekili.DB.profile.enabled or not Zekili:IsValidSpec() then
            return
        end
        ...
        local now = GetTime()
        local spec = state.spec.id and rawget( Zekili.DB.profile.specs, state.spec.id )
        ...
        for aura, targets in pairs( debuffs ) do
            ...
            for unit, entry in pairs( targets ) do
                ...
                elseif special == "combatExit" and not friendly then
                    -- Zekili:Error( format( "Auditor removed an aura %d from %s after exiting combat.", aura, unit ) )
                    ns.trackDebuff( aura, unit )
                end
            end
        end
    ...
    Zekili.AuditTimer = C_Timer.NewTicker( 1, ns.Audit )
end
...
do
    ...
    function Zekili:GetDeathClockByGUID( guid )
    ...
    function Zekili:GetTTD( unit, isGUID )
    ...
    function Zekili:GetTimeToPct( unit, percent )
    ...
    function Zekili:GetTimeToPctByGUID( guid, percent )
        if percent >= 1 then
            percent = percent / 100
        end
    ...
    function Zekili:GetGreatestTTD()
    ...
    function Zekili:GetGreatestTimeToPct( percent )
        local time, validUnit, now = 0, false, GetTime()
    ...
    function Zekili:GetNumTTDsWithin( x )
    ...
    Zekili.GetNumTTDsBefore = Zekili.GetNumTTDsWithin

    function Zekili:GetNumTTDsAfter( x )
        local count = 0
        local now = GetTime()
    ...
    function Zekili:GetNumTargetsAboveHealthPct( amount, inclusive, minTTD )
        local count, now = 0, GetTime()
    ...
    function Zekili:GetNumTargetsBelowHealthPct( amount, inclusive, minTTD )
        amount = amount > 1 and ( amount / 100 ) or amount
        inclusive = inclusive or false
        minTTD = minTTD or 3
    ...
    function Zekili:ExpireTTDs( all )
        local now = GetTime()
    ...
    Zekili:ProfileCPU( "UpdateTTDs", UpdateTTDs )
