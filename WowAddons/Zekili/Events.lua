...
Zekili:ProfileFrame( "GeneralEvents", events )
local handlers = {}
local unitHandlers = {}
...
function Zekili:GetActiveDisplays()
    return activeDisplays
end


local handlerCount = {}
Zekili.ECount = handlerCount
Zekili.IC = itemCallbacks

local eventData = {}
Zekili.EData = eventData
...
function ns.StartEventHandler()
    ...
    events:SetScript( "OnUpdate", function( self, elapsed )
        if Zekili.PendingSpecializationChange then
...
ns.RegisterEvent = function( event, handler )
    ...
    Zekili:ProfileCPU( key, handler )

    local stack = debugstack(2)
    local file, line = stack:match([[Zekili/(.-)"%]:(%d+)]])
    Zekili.EventSources[ key ] = file and ( file .. ":" .. ( line or 0 ) ) or stack:match( "^(.*)\n" )
end
...
ns.RegisterUnitEvent = function( event, unit1, unit2, handler )
    ...
    if not unitHandlers[ unit1 ] then
        ...
        Zekili:ProfileFrame( "UnitEvents:" .. unit1, unitHandlers[ unit1 ] )
    ...
    local stack = debugstack(2)
    local file, line = stack:match([[Zekili/(.-)"%]:(%d+)]])
    Zekili.EventSources[ event .. "_" .. unit1 .. "_" .. #unitFrame.events[ event ] ] = file and ( file .. ":" .. ( line or 0 ) ) or stack:match( "^(.*)\n" )

    unitFrame:RegisterUnitEvent( event, unit1 )
    Zekili:ProfileCPU( event .. "_" .. unit1 .. "_" .. #unitFrame.events[ event ], handler )

    if unit2 then
        if not unitHandlers[ unit2 ] then
            unitHandlers[ unit2 ] = CreateFrame( "Frame" )
            Zekili:ProfileFrame( "UnitEvents:" .. unit2, unitHandlers[ unit2 ] )
        ...
        Zekili.EventSources[ event .. "_" .. unit2 .. "_" .. #unitFrame.events[ event ] ] = file and ( file .. ":" .. ( line or 0 ) ) or stack:match( "^(.*)\n" )

        unitFrame:RegisterUnitEvent( event, unit2 )
        Zekili:ProfileCPU( event .. "_" .. unit2 .. "_" .. #unitFrame.events[ event ], handler )
    end
end
...
Zekili.FeignEvent = ns.FeignEvent
...
do
    ...
    function Zekili:ContinueOnSpellLoad( spellID, func )
    ...
    function Zekili:RunSpellCallbacks()
...
RegisterEvent( "PLAYER_ENTERING_WORLD", function( event, login, reload )
    if not Zekili.PLAYER_ENTERING_WORLD and ( login or reload ) then
        Zekili.PLAYER_ENTERING_WORLD = true
        Zekili:SpecializationChanged()
        Zekili:RestoreDefaults()
...
RegisterEvent( "PLAYER_CONTROL_LOST", function ()
    Zekili:After( 0.1, Zekili.UpdateDisplayVisibility, Zekili )
end )
...
do
                ...
                for tier, tierInfo in ipairs( tiers ) do
                    for _, power in ipairs( tierInfo.azeritePowerIDs ) do
                        ...
                        if IsPowerSelected( loc, power ) then
                            ...
                            if not name then
                                Zekili:Error( "Missing Azerite Power info for #" .. pInfo.spellID .. ": " .. GetSpellInfo( pInfo.spellID ) .. "." )
                            else
    ...
    Zekili:ProfileCPU( "updatePowers", ns.updatePowers )
...
do
    ...
    local function buildUseItemsList()
        ...
        if #state.items > 0 then
            for i, item in ipairs( state.items ) do
                if not Zekili:IsItemScripted( item ) then
                    insert( itemList, {
                        action = item,
                        enabled = true,
        ...
        class.essence_unscripted = ( class.active_essence and not Zekili:IsEssenceScripted( class.active_essence ) ) or false

        Zekili:LoadItemScripts()
    end

    function Zekili:UpdateUseItems()
        if not Zekili.PLAYER_ENTERING_WORLD then
    ...
    function Zekili:RegisterGearHook( r, u )
        insert( GearHooks, {
            reset = r,
            update = u
        } )
    ...
    local wasWearing = {}
    local maxItemSlot = Zekili.IsWrath() and INVSLOT_LAST_EQUIPPED or Enum.ItemSlotFilterTypeMeta.MaxValue
...
RegisterEvent( "PLAYER_REGEN_DISABLED", function( event )
    ...
    if Zekili.Config and not LibStub( "AceConfigDialog-3.0" ).OpenFrames[ "Zekili" ] then
        ns.StopConfiguration()
        Zekili:UpdateDisplayVisibility()
    end

    -- Zekili:ExpireTTDs( true )
    Zekili:ForceUpdate( event ) -- Force update on entering combat since OOC refresh can be very slow (0.5s).
end )
...
do
    ...
    function Zekili:GetMacroCastTarget( spell, castTime, source )
...
RegisterUnitEvent( "UNIT_SPELLCAST_SUCCEEDED", "player", "target", function( event, unit, _, spellID )
    if not noClassWarned and not class.initialized then
        Zekili:Notify( UnitClass( "player" ) .. " does not have any Zekili modules loaded (yet).\nWatch for updates.", 5 )
        noClassWarned = true
    elseif not lowLevelWarned and UnitLevel( "player" ) < 50 then
        Zekili:Notify( "Zekili is designed for current content.\nUse below level 50 at your own risk.", 5 )
        lowLevelWarned = true
    end
...
RegisterUnitEvent( "UNIT_SPELLCAST_DELAYED", "player", nil, function( event, unit, _, spellID )
    ...
    if ability then
        ...
        if start and finish then
            if not target then target = Zekili:GetMacroCastTarget( action, start / 1000, "DELAYED" ) end
            ...
            if ability.isProjectile then
                ...
                elseif target then
                    local u = Zekili:GetUnitByGUID( target ) or Zekili:GetNameplateUnitForGUID( target ) or "target"
...
RegisterEvent( "UNIT_SPELLCAST_SENT", function ( event, unit, target_name, castID, spellID )
    ...
    local gubn = Zekili:GetUnitByName( target_name )
...
function Zekili:UpdateDamageDetectionForCLEU()
...
-- Note that this was ported from an unreleased version of Zekili, and is currently only counting damaged enemies.
            ...
            if ability then
                if subtype == "SPELL_CAST_START" then
                    ...
                    if destGUID:len() == 0 then
                        destGUID = Zekili:GetMacroCastTarget( ability.key, GetTime(), "START" ) or UnitGUID( "target" )
                    end
                    ...
                    if start then
                        ...
                        if ability.isProjectile then
                            ...
                            elseif destGUID then
                                local unit = Zekili:GetUnitByGUID( destGUID ) or Zekili:GetNameplateUnitForGUID( destGUID ) or "target"
                ...
                elseif subtype == "SPELL_AURA_REMOVED" and ability.channeled then
                    ...
                    -- Zekili:ForceUpdate( "SPELL_AURA_REMOVED_CHANNEL" )
                ...
                elseif subtype == "SPELL_DAMAGE" then
                    ...
                    if state:RemoveSpellEvent( ability.key, true, "PROJECTILE_IMPACT" ) then
                        Zekili:ForceUpdate( "PROJECTILE_IMPACT" )
                    end
                end
            end

            state.gcd.lastStart = max( state.gcd.lastStart, ( GetSpellCooldown( 61304 ) ) )
            -- if subtype ~= "SPELL_DAMAGE" then Zekili:ForceUpdate( subtype, true ) end
        end
    end
    ...
    elseif ( amSource or petSource ) or ( countPets and minion ) or ( sourceGUID == destGUID and sourceGUID == UnitGUID( 'target' ) ) then
        --[[ if aura_events[ subtype ] then
            if subtype == "SPELL_CAST_SUCCESS" or state.GUID == destGUID then
                if class.abilities[ spellID ] or class.auras[ spellID ] then
                    Zekili:ForceUpdate( subtype, true )
                end
            end

            if UnitGUID( 'target' ) == destGUID then
                if class.auras[ spellID ] then Zekili:ForceUpdate( subtype ) end
            end
        end ]]
            ...
            if hostile and sourceGUID ~= destGUID and not aura.friendly then
                ...
                if subtype == 'SPELL_AURA_APPLIED' or subtype == 'SPELL_AURA_REFRESH' or subtype == 'SPELL_AURA_APPLIED_DOSE' then
                    ...
                    --[[ if spellID == 48108 or spellID == 48107 then
                        Zekili:ForceUpdate( "SPELL_AURA_SUPER", true )
                    end ]]
        ...
        if hostile and ( countDots and dmg_events[ subtype ] or direct_dmg_events[ subtype ] ) and not dmg_filtered[ spellID ] then
            ...
            if not IsResting() and ( ( ( subtype == "SPELL_DAMAGE" or subtype == "SPELL_PERIODIC_DAMAGE" ) and interrupt > 0 ) or ( subtype == "SWING_DAMAGE" and spellName > 0 ) ) and ns.isTarget( destGUID ) then
                ns.eliminateUnit( destGUID, true )
                -- Zekili:ForceUpdate( "SPELL_DAMAGE_OVERKILL" )
...
Zekili:ProfileCPU( "CLEU_HANDLER", CLEU_HANDLER )
...
local function ReadKeybindings( event )
        if not Zekili:IsValidSpec() then return end
        ...
        Zekili:SetupPetBasedTargetDetection()
end
...
if select( 2, UnitClass( "player" ) ) == "DRUID" then
    ...
    function Zekili:GetBindingForAction( key, display, i )
