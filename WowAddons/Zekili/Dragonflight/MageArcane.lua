...
local spec = Zekili:NewSpecialization( 62 )
...
do
    ...
    function Zekili:EmbedDisciplinaryCommand( x )
        local file_id = x.id
...
spec:RegisterHook( "reset_precast", function ()
    ...
    if Zekili.ActiveDebug then Zekili:Debug( "Arcane Phases (reset): aoe_spark_phase[%s], spark_phase[%s], rop_phase[%s]", tostring( virtualAoeSparkPhase ), tostring( virtualSparkPhase ), tostring( virtualRopPhase ) ) end
end )
...
spec:RegisterHook( "runHandler", function()
    ...
    if Zekili.ActiveDebug then Zekili:Debug( "Arcane Phases (handler): aoe_spark_phase[%s], spark_phase[%s], rop_phase[%s]", tostring( virtualAoeSparkPhase ), tostring( virtualSparkPhase ), tostring( virtualRopPhase ) ) end
end )
...
spec:RegisterHook( "advance", function()
    ...
    if Zekili.ActiveDebug then Zekili:Debug( "Arcane Phases (advance): aoe_spark_phase[%s], spark_phase[%s], rop_phase[%s]", tostring( virtualAoeSparkPhase ), tostring( virtualSparkPhase ), tostring( virtualRopPhase ) ) end
end )
