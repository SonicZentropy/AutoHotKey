...
if not Zekili.IsDragonflight() then return end
...
local state, class = Zekili.State, Zekili.Class
    ...
    function ns.updateTalents()
        ...
        for token, data in pairs( class.talents ) do
            ...
            if data[3] and node and node.maxRanks > 0 and node.maxRanks ~= data[3] then
                Zekili:Error( "Talent '%s' model expects %d ranks but actual max ranks was %d.", token, data[3], node.maxRanks )
            end
