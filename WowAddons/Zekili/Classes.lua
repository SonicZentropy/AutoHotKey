...
local ZekiliSpecMixin = {
    ...
    RegisterAura = function( self, aura, data )
        ...
        if a.id then
            if a.id > 0 then
                -- Zekili:ContinueOnSpellLoad( a.id, function( success )
                a.onLoad = function( a )
                    ...
                    if not a.name then
                        ...
                        Zekili.InvalidSpellIDs = Zekili.InvalidSpellIDs or {}
                        Zekili.InvalidSpellIDs[ a.id ] = a.name or a.key
            ...
            if hook == "reset_precast" then
                self:RegisterHook( hook, function()
                    ...
                    if state.time == 0 and not InCombatLockdown() then
                        phase.real[ d ] = false
                        -- Zekili:Print( format( "[ %s ] Phase '%s' set to '%s' (%s) - out of combat.", self.name or "Unspecified", key, tostring( phase.real[ d ] ), hook ) )
                        -- if Zekili.ActiveDebug then Zekili:Debug( "[ %s ] Phase '%s' set to '%s' (%s) - out of combat.", self.name or "Unspecified", key, tostring( phase.virtual[ display or "Primary" ] ), hook ) end
                    end
                    ...
                    --[[ if phase.real[ d ] ~= original then
                        if d == "Primary" then Zekili:Print( format( "Phase change for %s [ %s ] (from %s to %s).", key, d, tostring( original ), tostring( phase.real[ d ] ) ) ) end
                    end ]]

                    phase.virtual[ d ] = phase.real[ d ]

                    if Zekili.ActiveDebug then Zekili:Debug( "[ %s ] Phase '%s' set to '%s' (%s).", self.name or "Unspecified", key, tostring( phase.virtual[ d ] ), hook ) end
                end )
            else
                self:RegisterHook( hook, function()
                    local d = display or "Primary"
                    ...
                    if Zekili.ActiveDebug and phase.virtual[ d ] ~= previous then Zekili:Debug( "[ %s ] Phase '%s' set to '%s' (%s) - virtual.", self.name or "Unspecified", key, tostring( phase.virtual[ d ] ), hook ) end
                end )
            end
        end
            ...
            if hook == "reset_precast" then
                self:RegisterHook( hook, function()
                    ...
                    if Zekili.ActiveDebug then
                        Zekili:Debug( "[ %s ] Phased variable '%s' set to '%s' (%s) - was '%s'.", self.name or "Unspecified", key, tostring( phase.virtual[ display or "Primary" ] ), hook, tostring( original ) )
                    end
                end )
            else
                self:RegisterHook( hook, function()
                    ...
                    if Zekili.ActiveDebug and phase.virtual[ d ] ~= previous then Zekili:Debug( "[ %s ] Phased variable '%s' set to '%s' (%s) - virtual.", self.name or "Unspecified", key, tostring( phase.virtual[ display or "Primary" ] ), hook ) end
                end )
            end
        end
    ...
    RegisterAbility = function( self, ability, data )
        ...
        if data.id and type( data.id ) == "function" then
            if not data.copy or type( data.copy ) == "table" and #data.copy == 0 then
                Zekili:Error( "RegisterAbility for %s (Specialization %d) will fail; ability has an ID function but needs to have 'copy' entries for the abilities table.", ability, self.id )
            end
        end
        ...
        if data.hasteCD then
            if type( data.cooldown ) == "number" and data.cooldown > 0 then data.cooldown = Zekili:Loadstring( "return " .. data.cooldown .. " * haste" ) end
            if type( data.recharge ) == "number" and data.recharge > 0 then data.recharge = Zekili:Loadstring( "return " .. data.recharge .. " * haste" ) end
        end

        if not data.fixedCast and type( data.cast ) == "number" then
            data.cast = Zekili:Loadstring( "return " .. data.cast .. " * haste" )
        end
        ...
        if a.id and a.id > 0 then
            ...
            a.onLoad = function()
                ...
                if not a.name then
                    ...
                    Zekili.InvalidSpellIDs = Zekili.InvalidSpellIDs or {}
                    table.insert( Zekili.InvalidSpellIDs, a.id )
                    Zekili:Error( "Name info not available for " .. a.id .. "." )
                    return
                end

                if not a.name then Zekili:Error( "Name info not available for " .. a.id .. "." ); return false end
                ...
                if a.rangeSpell and type( a.rangeSpell ) == "number" then
                    Zekili:ContinueOnSpellLoad( a.rangeSpell, function( success )
                        if success then
    ...
    GetSetting = function( self, info )
        ...
        return Zekili.DB.profile.specs[ self.id ].settings[ setting ]
    end,

    SetSetting = function( self, info, val )
        local setting = info[ #info ]
        Zekili.DB.profile.specs[ self.id ].settings[ setting ] = val
    end,
    ...
    RegisterSetting = function( self, key, value, option )
        ...
        option.get = option.get or function( info )
            local setting = info[ #info ]
            local val = Zekili.DB.profile.specs[ self.id ].settings[ setting ]
...
function Zekili:RestoreDefaults()
    ...
    if #changed > 0 then
        ...
        if msg then
            C_Timer.After( 5, function()
                if Zekili.DB.profile.notifications.enabled then Zekili:Notify( msg, 6 ) end
                Zekili:Print( msg )
            end )
        end
    end
...
function Zekili:RestoreDefault( name )
    local p = self.DB.profile
...
function Zekili:NewSpecialization( specID, isRanged, icon )
    ...
    if Zekili.IsRetail() and specID > 0 then id, name, _, texture, role, pClass = GetSpecializationInfoByID( specID )
    else
        id = specID
        texture = icon
    end
    ...
    if not id then
        Zekili:Error( "Unable to generate specialization DB for spec ID #" .. specID .. "." )
        return nil
    end
    ...
    for key, func in pairs( ZekiliSpecMixin ) do
        spec[ key ] = func
    end
...
local all = Zekili:NewSpecialization( 0, "All", "Interface\\Addons\\Zekili\\Textures\\LOGO-WHITE.blp" )
                ...
                if spell then
                    ...
                    if unit == "target" and Zekili.DB.profile.filterCasts then
                        ...
                        if npcid and filters[ npcid ] and not filters[ npcid ][ spellID ] then
                            if Zekili.ActiveDebug then Zekili:Debug( "Cast '%s' ignored per user preference.", spell ) end
            ...
            for i, item in ipairs( potion.items ) do
                ...
                if not each_potion:IsItemEmpty() then
                    each_potion:ContinueOnItemLoad( function()
                        ...
                        if not spell then Zekili:Error( "No spell found for item %d.", item ) return false end
                    ...
                    Zekili:Error( "Item %d is empty.", item )
                end
            end
        end )
    end
...
do
    ...
    all:RegisterStateExpr( "coral_time_to_30", function()
        ...
        return Zekili:GetTimeToPctByGUID( coralGUID, 30 ) - ( offset + delay )
    end ) ]]
        ...
        razor_coral = {
            ...
            generate = function( t, auraType )
                ...
                --[[ elseif coralGUID ~= "none" then
                    ...
                    t.expires = coralApplied > 0 and ( coralApplied + 120 ) or ( state.query_time + Zekili:GetDeathClockByGUID( coralGUID ) )
                    t.caster = "player"
...
ns.addToggle = function( name, default, optionName, optionDesc )
    ...
    if Zekili.DB.profile[ 'Toggle State: ' .. name ] == nil then
        Zekili.DB.profile[ 'Toggle State: ' .. name ] = default
    end
...
ns.addSetting = function( name, default, options )
    ...
    if Zekili.DB.profile[ 'Class Option: ' .. name ] == nil then
        Zekili.DB.profile[ 'Class Option: ' ..name ] = default
    end
...
local function modifyElement( t, k, elem, value )
    ...
    if type( value ) == "function" then
        entry.mods[ elem ] = setfenv( value, Zekili.State )
    else
...
function Zekili:GetAbilityInfo( index )
...
function Zekili:GetActiveSpecOption( opt )
    if not self.currentSpecOpts then return end
    return self.currentSpecOpts[ opt ]
end


function Zekili:GetActivePack()
    return self:GetActiveSpecOption( "package" )
end


Zekili.SpecChangeHistory = {}

function Zekili:SpecializationChanged()
        ...
        if spec then
            ...
            for name, func in pairs( spec.stateFuncs ) do
                if not class.stateFuncs[ name ] then
                    if rawget( state, name ) then
                        Zekili:Error( "Cannot RegisterStateFunc for an existing expression ( " .. spec.name .. " - " .. name .. " )." )
                    else
                        class.stateFuncs[ name ] = func
                        rawset( state, name, func )
                        -- Zekili:Error( "Not real error, registered " .. name .. " for " .. spec.name .. " (RSF)." )
                    end
                end
            end
            ...
            for name, t in pairs( spec.stateTables ) do
                if not class.stateTables[ name ] then
                    if rawget( state, name ) then
                        Zekili:Error( "Cannot RegisterStateTable for an existing expression ( " .. spec.name .. " - " .. name .. " )." )
                    else
                        class.stateTables[ name ] = t
                        rawset( state, name, t )
                        -- Zekili:Error( "Not real error, registered " .. name .. " for " .. spec.name .. " (RST)." )
                    end
                end
            end

            if spec.id > 0 then
                local s = rawget( Zekili.DB.profile.specs, spec.id )
