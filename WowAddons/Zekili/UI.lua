...
local function stopScreenMovement(frame)
    ...
    local scale, pScale = Zekili:GetScale(), UIParent:GetScale()
...
local function Mover_OnMouseUp(self, btn)
    ...
    elseif btn == "RightButton" then
        if obj:GetName() == "ZekiliNotification" then
            LibStub( "AceConfigDialog-3.0" ):SelectGroup( "Zekili", "displays", "nPanel" )
            return
        elseif obj and obj.id then
            LibStub( "AceConfigDialog-3.0" ):SelectGroup( "Zekili", "displays", obj.id )
            return
        end
    end
end
...
local function Mover_OnMouseDown( self, btn )
    ...
    if Zekili.Config and btn == "LeftButton" and not obj.Moving then
...
local function Button_OnMouseUp( self, btn )
    ...
    local mover = _G[ "ZekiliDisplay" .. display ]
    ...
    elseif (btn == "RightButton") then
        ...
        local mouseInteract = Zekili.Pause or Zekili.Config
        for i = 1, #ns.UI.Buttons do
            for j = 1, #ns.UI.Buttons[i] do
                ns.UI.Buttons[i][j]:EnableMouse(mouseInteract)
            end
        end
        ns.UI.Notification:EnableMouse( Zekili.Config )
        -- Zekili:SetOption( { "locked" }, true )
        GameTooltip:Hide()
...
local function Button_OnMouseDown(self, btn)
    ...
    if Zekili.Config and btn == "LeftButton" and not mover.Moving then
...
function ns.StartConfiguration( external )
    ...
    local scaleFactor = Zekili:GetScale()
    local ccolor = RAID_CLASS_COLORS[select(2, UnitClass("player"))]

    -- Notification Panel
    ns.UI.Notification.Mover = ns.UI.Notification.Mover or CreateFrame( "Frame", "ZekiliNotificationMover", ns.UI.Notification, "BackdropTemplate" )
    ns.UI.Notification.Mover:SetAllPoints(ZekiliNotification)
    ...
    if not f.Header then
        f.Header = f:CreateFontString( "ZekiliNotificationHeader", "OVERLAY", "GameFontNormal" )
        local path = f.Header:GetFont()
        f.Header:SetFont( path, 18, "OUTLINE" )
    end
    f.Header:SetAllPoints( ZekiliNotificationMover )
    f.Header:SetText( "Notifications" )
    f.Header:SetJustifyH( "CENTER" )
    f.Header:Show()

    if ZekiliNotificationMover:GetFrameLevel() > ZekiliNotification:GetFrameLevel() then
        local orig = ZekiliNotificationMover:GetFrameLevel()
        ZekiliNotification:SetFrameLevel(orig)
        ZekiliNotificationMover:SetFrameLevel(orig-1)
    end

    ns.UI.Notification:EnableMouse( true )
    ns.UI.Notification:SetMovable( true )

    ZekiliNotification:SetScript( "OnMouseDown", Mover_OnMouseDown )
    ZekiliNotification:SetScript( "OnMouseUp", Mover_OnMouseUp )
    ZekiliNotification:SetScript( "OnEnter", function( self )
        local H = Zekili

        if H.Config then
            Tooltip:SetOwner( self, "ANCHOR_TOPRIGHT" )

            Tooltip:SetText( "Zekili: Notifications" )
            Tooltip:AddLine( "Left-click and hold to move.", 1, 1, 1 )
            Tooltip:AddLine( "Right-click to open Notification panel settings.", 1, 1, 1 )
            Tooltip:Show()
        end
    end )
    ZekiliNotification:SetScript( "OnLeave", function(self)
        Tooltip:Hide()
    end )

    Zekili:ProfileFrame( "NotificationFrame", ZekiliNotification )
    ...
    for i, v in pairs( ns.UI.Displays ) do
        ...
        if ns.UI.Buttons[ i ][ 1 ] and Zekili.DB.profile.displays[ i ] then
            -- if not Zekili:IsDisplayActive( i ) then v:Show() end
            ...
            if Zekili:IsDisplayActive( v.id, true ) then
            ...
            if not v.Header then
                v.Header = v.Backdrop:CreateFontString( "ZekiliDisplay" .. i .. "Header", "OVERLAY", "GameFontNormal" )
    ...
    if not external then
        if not Zekili.OptionsReady then Zekili:RefreshOptions() end

        local ACD = LibStub( "AceConfigDialog-3.0" )
        ACD:SetDefaultSize( "Zekili", 800, 608 )
        ACD:Open( "Zekili" )

        local oFrame = ACD.OpenFrames["Zekili"].frame
        ...
        ns.OnHideFrame:SetScript( "OnHide", function(self)
            ...
            if not InCombatLockdown() then
                ...
                C_Timer.After( 0, function() Zekili:UpdateDisplayVisibility() end )
            end
        end )

        if not ns.OnHideFrame.firstTime then
            ACD:SelectGroup( "Zekili", "packs" )
            ACD:SelectGroup( "Zekili", "displays" )
            ACD:SelectGroup( "Zekili", "displays", "Multi" )
            ACD:SelectGroup( "Zekili", "general" )
            ns.OnHideFrame.firstTime = true
        end

        Zekili:ProfileFrame( "CloseOptionsFrame", ns.OnHideFrame )
    end

    Zekili:UpdateDisplayVisibility()
end

function Zekili:OpenConfiguration()
...
function ns.StopConfiguration()
    ...
    local scaleFactor = Zekili:GetScale()
    local mouseInteract = Zekili.Pause

    for id, display in pairs( Zekili.DisplayPool ) do
    ...
    ZekiliNotification:EnableMouse( false )
    ZekiliNotification:SetMovable( false )
    ZekiliNotification.Mover:Hide()
    -- ZekiliNotification.Mover.Header:Hide()
end
...
do
    ns.UI.Menu = ns.UI.Menu or CreateFrame( "Frame", "ZekiliMenu", UIParent, "UIDropDownMenuTemplate" )
    local menu = ns.UI.Menu

    Zekili:ProfileFrame( "ZekiliMenu", menu )
    ...
    local function SetDisplayMode( mode )
        Zekili.DB.profile.toggles.mode.value = mode
        if WeakAuras and WeakAuras.ScanEvents then WeakAuras.ScanEvents( "ZEKILI_TOGGLE", "mode", mode ) end
        if ns.UI.Minimap then ns.UI.Minimap:RefreshDataText() end

        Zekili:UpdateDisplayVisibility()
        Zekili:ForceUpdate( "ZEKILI_TOGGLE", true )
    end

    local function IsDisplayMode( p, mode )
        return Zekili.DB.profile.toggles.mode.value == mode
    end
    ...
    local menuData = {
        ...
        {
            text = "Enable",
            func = function () Zekili:Toggle() end,
            checked = function () return Zekili.DB.profile.enabled end,
        },

        {
            text = "Pause",
            func = function () return Zekili:TogglePause() end,
            checked = function () return Zekili.Pause end,
        },
        ...
        {
            text = "Cooldowns",
            func = function() Zekili:FireToggle( "cooldowns" ); ns.UI.Minimap:RefreshDataText() end,
            checked = function () return Zekili.DB.profile.toggles.cooldowns.value end,
        },

        {
            text = "Minor CDs",
            func = function() Zekili:FireToggle( "essences" ); ns.UI.Minimap:RefreshDataText() end,
            checked = function () return Zekili.DB.profile.toggles.essences.value end,
        },

        {
            text = "Interrupts",
            func = function() Zekili:FireToggle( "interrupts" ); ns.UI.Minimap:RefreshDataText() end,
            checked = function () return Zekili.DB.profile.toggles.interrupts.value end,
        },

        {
            text = "Defensives",
            func = function() Zekili:FireToggle( "defensives" ); ns.UI.Minimap:RefreshDataText() end,
            checked = function () return Zekili.DB.profile.toggles.defensives.value end,
        },
    ...
    function menu:initialize( level, list )
        ...
        if level == 1 then
            if not specsParsed then
                ...
                for i, spec in pairs( Zekili.Class.specs ) do
                    if i > 0 then
                        insert( menuData, {
                            isSeparator = 1,
                        ...
                        insert( menuData, {
                            text = "|TInterface\\Addons\\Zekili\\Textures\\Cycle:0|t Recommend Target Swaps",
                            tooltipTitle = "|TInterface\\Addons\\Zekili\\Textures\\Cycle:0|t Recommend Target Swaps",
                            tooltipText = "If checked, the |TInterface\\Addons\\Zekili\\Textures\\Cycle:0|t indicator may be displayed which means you should use the ability on a different target.",
                            tooltipOnButton = true,
                            func = function ()
                                local spec = rawget( Zekili.DB.profile.specs, i )
                                if spec then
                                    spec.cycle = not spec.cycle
                                    if Zekili.DB.profile.notifications.enabled then
                                        Zekili:Notify( "Recommend Target Swaps: " .. ( spec.cycle and "ON" or "OFF" ) )
                                    else
                                        Zekili:Print( "Recommend Target Swaps: " .. ( spec.cycle and " |cFF00FF00ENABLED|r." or " |cFFFF0000DISABLED|r." ) )
                                    end
                                end
                            end,
                            checked = function ()
                        ...
                        for n, setting in pairs( spec.settings ) do
                            if setting.info and ( not setting.info.arg or setting.info.arg() ) then
                                if setting.info.type == "toggle" then
                                    ...
                                    submenu = {
                                        ...
                                        func = function ()
                                            ...
                                            if Zekili.DB.profile.notifications.enabled then
                                                Zekili:Notify( nm .. ": " .. ( setting.info.get( menu.args ) and "ON" or "OFF" ) )
                                            else
                                                Zekili:Print( nm .. ": " .. ( setting.info.get( menu.args ) and " |cFF00FF00ENABLED|r." or " |cFFFF0000DISABLED|r." ) )
                                            end
                                            ...
                                            for _, k in orderedPairs( setting.info.sorting ) do
                                                ...
                                                insert( submenu.menuList, {
                                                    ...
                                                    func = function ()
                                                        ...
                                                        for k, v in pairs( Zekili.DisplayPool ) do
                                                            v:OnEvent( "ZEKILI_MENU" )
                                                        end
                                                    end,
                                                    checked = function ()
                                                        menu.args[1] = setting.name
                                ...
                                elseif setting.info.type == "range" then
                                    ...
                                    local cn = "ZekiliSpec" .. i .. "Option" .. n
                                    local cf = CreateFrame( "Frame", cn, UIParent, "ZekiliPopupDropdownRangeTemplate" )
                                    ...
                                    for j = low, high, step do
                                        ...
                                        insert( submenu.menuList, {
                                            ...
                                            func = function ()
                                                ...
                                                if Zekili.DB.profile.notifications.enabled then
                                                    Zekili:Notify( name .. " set to |cFF00FF00" .. actual .. "|r." )
                                                else
                                                    Zekili:Print( name .. " set to |cFF00FF00" .. actual .. "|r." )
                                                end
                                            end,
                                            checked = function ()
                                                menu.args[1] = setting.name
        ...
        local classic = Zekili.IsClassic()
...
do
    ...
    local function CalculateAlpha( id )
        if IsInPetBattle() or Zekili.Barber or Zekili.ClientScene or UnitHasVehicleUI( "player" ) or HasVehicleActionBar() or HasOverrideActionBar() or UnitOnTaxi( "player" ) or not Zekili:IsDisplayActive( id ) then
            return 0
        end

        local prof = Zekili.DB.profile
    ...
    function Zekili:CreateDisplay( id )
        local conf = rawget( self.DB.profile.displays, id )
        if not conf then return end

        if not dPool[ id ] then
            numDisplays = numDisplays + 1
            dPool[ id ] = CreateFrame( "Frame", "ZekiliDisplay" .. id, UIParent )
            dPool[ id ].index = numDisplays

            Zekili:ProfileFrame( "ZekiliDisplay" .. id, dPool[ id ] )
        end
        local d = dPool[ id ]
        ...
        function d:OnUpdate( elapsed )
            if not self.Recommendations or not Zekili.PLAYER_ENTERING_WORLD then
                return
            end
            ...
            if Zekili.Pause and not self.paused then
                self.Buttons[ 1 ].Overlay:Show()
                self.paused = true
            elseif not Zekili.Pause and self.paused then
                        ...
                        if ability then
                            ...
                            if conf.indicators.enabled and indicator then
                                if indicator == "cycle" then
                                    b.Icon:SetTexture("Interface\\Addons\\Zekili\\Textures\\Cycle")
                                end
                                if indicator == "cancel" then
                                    b.Icon:SetTexture("Interface\\Addons\\Zekili\\Textures\\Cancel")
                                end
                                b.Icon:Show()
                            else
                                b.Icon:Hide()
                    ...
                    if a and ( changed or self.flashTimer < 0 ) then
                        ...
                        if ability.item then
                            ...
                            elseif conf.flash.suppress and not self.flashWarnings[ iname ] then
                                ...
                                -- Zekili:Error( "|cffff0000WARNING|r - Could not flash recommended item '" .. iname .. "' (" .. self.id .. ")." )
                            end
                        else
                            ...
                            if aFlash then
                                ...
                                elseif conf.flash.suppress and not self.flashWarnings[ aFlash ] then
                                    ...
                                    -- Zekili:Error( "|cffff0000WARNING|r - Could not flash recommended action '" .. aFlash .. "' (" .. self.id .. ")." )
                                end
                            else
                                ...
                                if sname then
                                    ...
                                    elseif not self.flashWarnings[ sname ] then
                                        ...
                                        -- Zekili:Error( "|cffff0000WARNING|r - Could not flash recommended ability '" .. sname .. "' (" .. self.id .. ")." )
                                    end
                                end
                            end
                        end
        ...
        Zekili:ProfileCPU( "ZekiliDisplay" .. id .. ":OnUpdate", d.OnUpdate )
        ...
        function d:UpdateAlpha()
            ...
            if preAlpha > 0 and newAlpha == 0 then
                ...
                if preAlpha == 0 and newAlpha > 0 then
                    Zekili:ForceUpdate( "DISPLAY_ALPHA_CHANGED:" .. d.id .. ":" .. preAlpha .. ":" .. newAlpha .. ":" .. GetTime() )
                end
        ...
        function d:OnEvent( event, ... )
            ...
            elseif alphaUpdateEvents[ event ] then
                if event == "CLIENT_SCENE_OPENED" then
                    if ... == 1 then -- Minigame.
                        Zekili.ClientScene = true
                    end
                elseif event == "CLIENT_SCENE_CLOSED" then
                    Zekili.ClientScene = nil
                end
        ...
        Zekili:ProfileCPU( "ZekiliDisplay" .. id .. ":OnEvent", d.OnEvent )
    ...
    function Zekili:CreateCustomDisplay( id )
        local conf = rawget( self.DB.profile.displays, id )
        if not conf then return end

        dPool[ id ] = dPool[ id ] or CreateFrame( "Frame", "ZekiliDisplay" .. id, UIParent )
        local d = dPool[ id ]
        self:ProfileFrame( "ZekiliDisplay" .. id, d )
        ...
        d.Activate = ZekiliDisplayPrimary.Activate
        d.Deactivate = ZekiliDisplayPrimary.Deactivate
        d.RefreshCooldowns = ZekiliDisplayPrimary.RefreshCooldowns
        d.UpdateAlpha = ZekiliDisplayPrimary.UpdateAlpha
        d.UpdateKeybindings = ZekiliDisplayPrimary.UpdateKeybindings
    ...
    function Zekili:UpdateDisplayVisibility()
    ...
    function Zekili:IsDisplayActive( display, config )
        if config then
    ...
    function Zekili:IsListActive( pack, list )
        return pack == "UseItems" or ( listActive[ pack .. ":" .. list ] == true )
    end

    function Zekili:IsActionActive( pack, list, action )
        return pack == "UseItems" or ( actsActive[ pack .. ":" .. list .. ":" .. action ] == true )
    end

    function Zekili:DumpActionActive()
        DevTools_Dump( actsActive )
    end


    -- Separate the recommendations engine from each display.
    Zekili.Engine = CreateFrame( "Frame", "ZekiliEngine" )

    Zekili.Engine.refreshTimer = 1
    Zekili.Engine.eventsTriggered = {}

    function Zekili.Engine:UpdatePerformance( wasted )
    ...
    Zekili.Engine:SetScript( "OnUpdate", function( self, elapsed )
        if not self.activeThread then
            self.refreshTimer = self.refreshTimer + elapsed
            insert( frameSpans, elapsed )
        end

        if Zekili.DB.profile.enabled and not Zekili.Pause then
            ...
            if superUpdate or ( not thread and self.refreshTimer > ( self.criticalUpdate and self.combatRate or self.refreshRate ) ) then
                ...
                self.activeThread = coroutine.create( Zekili.Update )
                self.activeThreadTime = 0
                ...
                if not self.firstThreadCompleted then
                    Zekili.maxFrameTime = InCombatLockdown() and 10 or 25
                else
                    if #frameSpans > 0 then
                        ...
                        Zekili.maxFrameTime = Clamp( 0.6 * averageSpan, 3, 20 ) -- Dynamically adjust to 60% of (seemingly) average frame rate between updates.
                    else
                        Zekili.maxFrameTime = Zekili.maxFrameTime or 10
                    end
                end

                --[[
                elseif Zekili:GetActiveSpecOption( "throttleTime" ) then
                    Zekili.maxFrameTime = Zekili:GetActiveSpecOption( "maxTime" ) or 15
                else
                    Zekili.maxFrameTime = 15
                end ]]
            ...
            if thread and coroutine.status( thread ) == "suspended" then
                ...
                Zekili.activeFrameStart = debugprofilestop()

                local ok, err = coroutine.resume( thread, firstDisplay )

                if not ok then
                    err = err .. "\n\n" .. debugstack( thread )
                    Zekili:Error( "Update: " .. err )

                    if Zekili.ActiveDebug then
                        Zekili:Debug( format( "Recommendation thread terminated due to error: %s", err and err:gsub( "%%", "%%%%" ) or "Unknown" ) )
                        Zekili:SaveDebugSnapshot( self.id )
                        Zekili.ActiveDebug = nil
                    end

                    pcall( error, err )
                end

                self.activeThreadTime = self.activeThreadTime + debugprofilestop() - Zekili.activeFrameStart

                if coroutine.status( thread ) == "dead" or err then
                    self.activeThread = nil

                    if Zekili:GetActiveSpecOption( "throttleRefresh" ) then
                        self.refreshRate = Zekili:GetActiveSpecOption( "regularRefresh" )
                        self.combatRate = Zekili:GetActiveSpecOption( "combatRefresh" )
                    else
                ...
                if ok and err == "AutoSnapshot" then
                    Zekili:MakeSnapshot( true )
                end
            end
        end
    end )
    Zekili:ProfileFrame( "ZekiliEngine", Zekili.Engine )


    function ZekiliEngine:IsThreadActive()
        return self.activeThread and coroutine.status( self.activeThread ) == "suspended"
    end


    function Zekili:ForceUpdate( event, super )
    ...
    function Zekili:CreateButton( dispID, id )
        ...
        local bName = "Zekili_" .. dispID .. "_B" .. id
        local b = d.Buttons[ id ] or CreateFrame( "Button", bName, d )

        Zekili:ProfileFrame( bName, b )
        ...
        elseif id == 2 then
            ...
            if queueAnchor:sub( 1, 5 ) == "RIGHT" then
                ...
                b:SetPoint( align .. getInverseDirection(dir), "Zekili_" .. dispID .. "_B1", align .. dir, ( borderOffset + qOffsetX ) * scale, qOffsetY * scale )
            elseif queueAnchor:sub( 1, 4 ) == "LEFT" then
                local dir, align = "LEFT", queueAnchor:sub(5)
                b:SetPoint( align .. getInverseDirection(dir), "Zekili_" .. dispID .. "_B1", align .. dir, -1 * ( borderOffset + qOffsetX ) * scale, qOffsetY * scale )
            elseif queueAnchor:sub( 1, 3)  == "TOP" then
                local dir, align = "TOP", queueAnchor:sub(4)
                b:SetPoint( getInverseDirection(dir) .. align, "Zekili_" .. dispID .. "_B1", dir .. align, 0, ( borderOffset + qOffsetY ) * scale )
            else -- BOTTOM
                local dir, align = "BOTTOM", queueAnchor:sub(7)
                b:SetPoint( getInverseDirection(dir) .. align, "Zekili_" .. dispID .. "_B1", dir .. align, 0, -1 * ( borderOffset + qOffsetY ) * scale )
            end
        else
            ...
            if queueDirection == "RIGHT" then
                b:SetPoint( getInverseDirection(queueDirection), "Zekili_" .. dispID .. "_B" .. id - 1, queueDirection, btnSpacing * scale, 0 )
            elseif queueDirection == "LEFT" then
                b:SetPoint( getInverseDirection(queueDirection), "Zekili_" .. dispID .. "_B" .. id - 1, queueDirection, -1 * btnSpacing * scale, 0 )
            elseif queueDirection == "TOP" then
                b:SetPoint( getInverseDirection(queueDirection), "Zekili_" .. dispID .. "_B" .. id - 1, queueDirection, 0, btnSpacing * scale )
            else -- BOTTOM
                b:SetPoint( getInverseDirection(queueDirection), "Zekili_" .. dispID .. "_B" .. id - 1, queueDirection, 0, -1 * btnSpacing * scale )
            end
        end
...
function Zekili:BuildUI()
    ...
    ns.UI.Keyhandler = ns.UI.Keyhandler or CreateFrame( "Button", "Zekili_Keyhandler", UIParent )
    ns.UI.Keyhandler:RegisterForClicks( "AnyDown" )
    ns.UI.Keyhandler:SetScript( "OnClick", function( self, button, down )
        Zekili:FireToggle( button )
    end )
    Zekili:ProfileFrame( "KeyhandlerFrame", ns.UI.Keyhandler )
    ...
    local f = ns.UI.Notification or CreateFrame( "Frame", "ZekiliNotification", UIParent )
    Zekili:ProfileFrame( "ZekiliNotification", f )
    ...
    f.Text = f.Text or f:CreateFontString( "ZekiliNotificationText", "OVERLAY" )
    ...
    --if Zekili.Config then ns.StartConfiguration() end
    if MasqueGroup then
        MasqueGroup:ReSkin()
    end
    ...
    for display, buttons in ipairs(ns.UI.Buttons) do
        if not Zekili.DB.profile.displays[display] then
...
function Zekili:SaveCoordinates()
    for i in pairs(Zekili.DB.profile.displays) do
    ...
    self.DB.profile.notifications.x, self.DB.profile.notifications.y = select( 4, ZekiliNotification:GetPoint() )
end
