...
local function forgetMeNots( str )
   ...
   while( str:find("!") ) do
      ...
      i = i + 1
      if i >= 100 then Zekili:Debug( "Was unable to convert '!' to 'not' in string [%s].", str ); break end
   end
...
local function atToAbs( str )
    ...
    while( str:find("@") ) do
       ...
       i = i + 1
       if i >= 100 then Zekili:Error( "Was unable to convert '@' to 'abs' in string [%s].", str ); break end
    end
            ...
            if piece and piece.t == "expr" then
                ...
                if ( prev and prev.t == "op" and math_ops[ prev.a ] and not equality[ prev.a ] ) or ( next and next.t == "op" and math_ops[ next.a ] and not equality[ next.a ] ) then
                    ...
                    if piece.s:find("^variable%.") then
                        ...
                        local func, warn = Zekili:Loadstring( "return " .. ( SimToLua( piece.s ) or "" ) )
                        if func then
                            setfenv( func, state )
                            ...
                            if not pass and not piece.s:match("variable") then
                                ...
                                Zekili:Error( "Unable to compile '" .. safepiece:gsub("%%", "%%%%") .. "' - " .. val .. " (pcall-n)\n\nFrom: " .. esString:gsub( "%%", "%%%%" ) )
                            else
                                if trimmed_prefix ~= "safenum" and ( val == nil or type( val ) ~= "number" ) then piece.s = "safenum(" .. piece.s .. ")" end
                            end
                        else
                            Zekili:Error( "Unable to compile '" .. ( piece.s ):gsub("%%","%%%%") .. "' - " .. warn .. " (loadstring-n)\nFrom: " .. esString:gsub( "%%", "%%%%" ) )
                        end
                    end
                    piece.r = nil
                ...
                elseif not numeric and ( not prev or ( prev.t == "op" and not math_ops[ prev.a ] and not equality[ prev.a ] ) ) and ( not next or ( next.t == "op" and not math_ops[ next.a ] and not equality[ next.a ] ) ) then
                    ...
                    if piece.s:find("^variable") then
                        ...
                        if func then
                            ...
                            if not pass and not piece.s:match("variable") then
                                local safepiece = piece.s:gsub( "%%", "%%%%" )
                                Zekili:Error( "Unable to compile '" .. safepiece:gsub("%%", "%%%%") .. "' - " .. val .. " (pcall-b)\nFrom: " .. esString:gsub( "%%", "%%%%" ) )
                            else
                            ...
                            Zekili:Error( "Unable to compile '" .. ( piece.s ):gsub("%%","%%%%") .. "' - " .. warn .. " (loadstring-b)." )
                        end
                    end
                    piece.r = nil
                end
...
local function GetScriptElements( script )
    ...
    for s in c:gmatch( "[^ ,]+" ) do
        ...
        if not e[ s ] and not tonumber( s ) then
            local ef = Zekili:Loadstring( "return ".. s )
            if ef then
                setfenv( ef, state )
...
local function generateDebugPrint( node, condition, header, isRecheck )
    ...
    if argn > 0 then
        ...
        debugPrint, formatArgs = Zekili:Loadstring( generateDebug )

        if formatArgs then
            Zekili:Error( "Unable to generate debug print for " .. header .. ": " .. formatArgs:gsub( "%%", "%%%%" ) .. "\n" .. generateDebug:gsub( "%%", "%%%%" ) )
            return
        end
...
local function ConvertScript( node, hasModifiers, header )
    ...
    if t then sf, e = Zekili:Loadstring( "-- " .. header .. "\nreturn safebool( " .. t .. " )" ) end
    ...
    if t and t ~= "" then
        ...
        if rs then
            local orig = rs
            rc, erc = Zekili:Loadstring( "-- " .. header .. " recheck\nreturn " .. rs )
            ...
            if type( rc ) ~= "function" then
                Zekili:Error( "Recheck function for " .. clean .. " ( " .. ( rs or "nil" ) .. ") was unsuccessful somehow." )
                rc = nil
            end
        end
    end
    ...
    if hasModifiers then
        for m, value in pairs( newModifiers ) do
            if node[ m ] then
                ...
                if node.action == "variable" then
                    ...
                    if var_val then
                        ...
                        if type( var_recheck ) ~= "function" then
                            Zekili:Error( "Variable recheck function for " .. node.criteria .. " ( " .. ( var_recheck or "nil" ) .. " ) was unsuccessful somehow." )
                            var_recheck = nil
                        end
                    ...
                    if rs then
                        local orig = rs
                        rc, erc = Zekili:Loadstring( "-- var " .. header .. " recheck\nreturn " .. rs )
                        if rc then setfenv( rc, state ) end
                        ...
                        if type( rc ) ~= "function" then
                            Zekili:Error( "Variable recheck function for " .. o .. " ( " .. ( rs or "nil" ) .. ") was unsuccessful somehow." )
                            rc = nil
                        end
                ...
                sf, e = Zekili:Loadstring( "return " .. emulated )
...
local function scriptLoader()
    if not Zekili.LoadingScripts and not scriptsLoaded then scripts:LoadScripts() end
end

function Zekili:ScriptsLoaded()
...
function scripts:LoadScripts()
        ...
        if specData then
            ...
            for list, lData in pairs( pData.lists ) do
                for action, data in ipairs( lData ) do
                    Zekili:Yield( "Loading " .. pack .. " - " .. list .. " - " .. action )
                    ...
                    if script.Error then
                        Zekili:Error( "Error in " .. scriptID .. " conditions:  " .. script.Error )
                    end
    ...
    Zekili.LoadingScripts = false
    scriptsLoaded = true
end


function Zekili:LoadScripts()
...
function Zekili:IsEssenceScripted( token )
...
function Zekili:IsItemScripted( token, specific )
    local pack = Zekili:GetActivePack()
    if not pack then return false end
...
function Zekili.Scripts:LoadItemScripts()
...
function Zekili:LoadItemScripts()
    self.Scripts:LoadItemScripts()
end


function Zekili:LoadScript( pack, list, id )
    ...
    if script.Error then
        Zekili:Error( "Error in " .. scriptID .. " conditions:  " .. script.SimC .. "\n    " .. script.Error )
    end
...
local function embedConditionsAndValues( source, elements )
    if source and source ~= "" then
        local wasDebugging = Zekili.ActiveDebug
        ...
        if wasDebugging then Zekili.ActiveDebug = true end
        return source
    end
...
do
    ...
    function scripts:GetConditionsAndValues( scriptID, listName, actID, recheck )
        if troubleshootingSnapshotTimes or not Zekili.ActiveDebug then return "[no data]" end
