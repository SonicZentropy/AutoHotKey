...
if Zekili.Version:match( "^Dev" ) then
    state.print = print
else
...
do
    local cycle = {}
    local debug = function( ... ) if Zekili.ActiveDebug then Zekili:Debug( ... ) end end
...
local function applyDebuff( unit, aura, duration, stacks, value, noPandemic )
    ...
    if state.cycle then
        if duration == 0 then
            if Zekili.ActiveDebug then Zekili:Debug( "Removed an application of '%s' while target-cycling.", aura ) end
            state.active_dot[ aura ] = state.active_dot[ aura ] - 1
        else
            if Zekili.ActiveDebug then Zekili:Debug( "Added an application of '%s' while target-cycling.", aura ) end
            state.active_dot[ aura ] = state.active_dot[ aura ] + 1
        end
        return
    end
...
local function timeToInterrupt()
    ...
    return max( 0, casting.remains - ( Zekili.DB.profile.toggles.interrupts.castRemainingThreshold or 0.25 ) )
end
...
function state.stopChanneling( reset, action )
    if not reset then
        ...
        if spell then
            if Zekili.ActiveDebug then Zekili:Debug( "Breaking channel of %s.", spell ) end
...
state.spell_targets = setmetatable( {}, {
    __index = function( t, k )
        ...
        if ability.max_ttd then n = min( n, Zekili:GetNumTTDsBefore( ability.max_ttd + state.offset + state.delay ) ) end
        if ability.min_ttd then n = min( n, Zekili:GetNumTTDsAfter( ability.min_ttd + state.offset + state.delay ) ) end
        ...
        for i = 1, n do
            ...
            if type( x ) == "number" then
                if x > 0 and x >= state.delayMin and x <= state.delayMax then
                    t[ x ] = true
                elseif Zekili.ActiveDebug and x < 60 then
                    Zekili:Debug( "Excluded %.2f recheck time as it is outside our constraints ( %.2f - %.2f ).", x, state.delayMin or -1, state.delayMax or -1 )
                end
            end
        end
    end
    ...
    function state.recheck( ability, script, stack, block )
        ...
        if script then
            ...
            if script.Variables then
                -- if Zekili.ActiveDebug then table.insert( steps, debugprofilestop() ) end
        ...
        for k, v in pairs( workTable ) do
            -- if Zekili.ActiveDebug then Zekili:Debug( "%s - %s", tostring( k ), tostring( v ) ) end
            times[ #times + 1 ] = k
        end
    ...
    mt_state = {
        __index = function( t, k )
            ...
            elseif k == "fight_remains" or k == "interpolated_fight_remains" or k == "time_to_die" then
                ...
                if not n then Zekili:Error( "longest_ttd was nil, GetGreatestTTD is " .. ( Zekili:GetGreatestTTD() or "nil" ) .. "." ) end
                return max( 1, t.longest_ttd - ( t.offset + t.delay ) )
            elseif k == "index" then t[k] = 0
            elseif k == "longest_ttd" then t[k] = Zekili:GetGreatestTTD()
            elseif k == "now" then t[k] = GetTime()
            elseif k == "offset" then t[k] = 0
            elseif k == "query_time" then  return t.now + t.offset + t.delay
            elseif k == "shortest_ttd" then t[k] = Zekili:GetLowestTTD()
            elseif k == "time" then
            ...
            elseif type(k) == "string" and k:sub(1, 12) == "time_to_pct_" then
                ...
                return Zekili:GetGreatestTimeToPct( percent ) - ( t.offset + t.delay )
            ...
            elseif k == "cycle_enemies" then
                ...
                targets = targets - Zekili:GetNumTTDsBefore( minTTD )

                if maxTTD then
                    targets = targets - Zekili:GetNumTTDsAfter( maxTTD )
                end
                ...
                -- if Zekili.ActiveDebug then Zekili:Debug( "cycle min:%.2f, max:%.2f, ae:%d, before:%d, after:%d, cycle_enemies:%d", minTTD or 0, maxTTD or 0, t.active_enemies, minTTD and Zekili:GetNumTTDsBefore( minTTD ) or 0, maxTTD and Zekili:GetNumTTDsAfter( maxTTD ) or 0, max( 1, targets ) ) end
...
do
    ...
    mt_target = {
        __index = function( t, k )
            ...
            elseif k == "real_ttd" then t[k] = Zekili:GetTTD( "target" )
            ...
            elseif k:sub(1, 12) == "time_to_pct_" then
                local percent = tonumber( k:sub( 13 ) ) or 0
                return Zekili:GetTimeToPct( "target", percent ) - ( state.offset + state.delay )
...
do
    ...
    mt_buffs = {
        ...
        __index = function( t, k )
            ...
            if not aura then
                if Zekili.PLAYER_ENTERING_WORLD and not buffs_warned[ k ] then
                    Zekili:Error( "Unknown buff in [" .. state.scriptID .. "]: " .. k .. "\n\n" .. debugstack() )
                    buffs_warned[ k ] = true
                end
                return unknown_buff
            end
...
Zekili.mt_default_totem = mt_default_totem
...
do
    ...
    state.variable = setmetatable( {
        ...
        __index = function( t, var )
            ...
            if Zekili.LoadingScripts then
                return defaultValue
            end

            if not db[ var ] then
                if debug then Zekili:Debug( "No such variable '%s'.", var ) end
                Zekili:Error( "Variable '%s' referenced in %s but is undefined.", var, state.scriptID )
                return defaultValue
            end
            ...
            --[[ if debug then
                Zekili:Debug( "%s Result = %s.", var, tostring( value ) )
            end ]]
    ...
    mt_debuffs = {
        ...
        __index = function( t, k )
            ...
            if aura then
                ...
                if Zekili.PLAYER_ENTERING_WORLD and not debuffs_warned[ k ] then
                    Zekili:Error( "Unknown debuff in [" .. ( state.scriptID or "unknown" ) .. "]: " .. k .. "\n\n" .. debugstack() )
...
do
    ...
    Zekili.StoreMatchingAuras = state.StoreMatchingAuras
    ...
    Zekili.ScrapeUnitAuras = state.ScrapeUnitAuras

    Zekili:ProfileCPU( "ScrapeUnitAuras", state.ScrapeUnitAuras )

    Zekili.AuraDB = ns.auras
end
...
do
    ...
    function state:QueueEvent( action, start, time, type, target, real )
        ...
        if action and start and time and type then
            ...
            if Zekili.ActiveDebug and not real then Zekili:Debug( "Queued %s from %.2f to %.2f (%s).", action, start, time, type ) end
        end
    end

    Zekili:ProfileCPU( "QueueEvent", state.QueueEvent )
    ...
    function state:QueueAuraEvent( action, func, time, eType, data )
        ...
        if Zekili.ActiveDebug then Zekili:Debug( "Queued %s %s at +%.2f.", action, eType, time - state.query_time ) end
    end
    ...
    function state:RemoveAuraEvent( action, eType )
        ...
        Zekili:Debug( "Trying to remove %s %s from queue.", action, eType )
        ...
        for i = 1, #queue do
            ...
            if e.action == action and e.type == eType then
                ...
                Zekili:Debug( "Removed #%d from queue.", i )
                break
            end
        end
    end
    ...
    function state:RemoveEvent( e, real )
        ...
        Zekili:Debug( "Trying to remove %s %s from queue.", ( e.action or "NO_ACTION" ), ( e.type or "NO_TYPE" ) )

        for i = #queue, 1, -1 do
            if queue[ i ] == e then
                Zekili:Debug( "Removing %d from queue.", i )
                RecycleEvent( queue, i )
    ...
    Zekili:ProfileCPU( "RemoveEvent", state.RemoveEvent )
    ...
    function state:HandleEvent( e )
        ...
        if e.type == "CAST_FINISH" then
            ...
            if e.target and e.target ~= self.target.unit then
                if Zekili.ActiveDebug then Zekili:Debug( "Using ability on a different target." ) end
    ...
    Zekili:ProfileCPU( "HandleEvent", state.HandleEvent )
...
do
    ...
    function state.reset( dispName, full )
        ...
        local displayFrame = _G[ "ZekiliDisplay" .. dispName ]
        ...
        -- Zekili:Yield( "Reset Post-States" )
        ...
        if state.time == 0 and InCombatLockdown() then
            ...
            if a and a.startsCombat and state.now - a.lastCast < 1 then
                ...
                if Zekili.ActiveDebug then Zekili:Debug( format( "Starting combat based on %s cast; time is now: %.2f.", state.player.lastcast, state.time ) ) end
            end
        end

        local foundResource = false

        -- Zekili:Yield( "Reset Pre-Powers" )
        ...
        -- Zekili:Yield( "Reset Post-Powers" )
        ...
        -- Zekili:Yield( "Reset Pre-Cast Hook" )
        ns.callHook( "reset_precast" )
        -- Zekili:Yield( "Reset Pre-Casting" )

        if state.empowerment.active then
            local timeDiff = state.now - state.empowerment.start
            if timeDiff > 0 then
                if Zekili.ActiveDebug then Zekili:Debug( "Empowerment is active; turning back time by " .. timeDiff .. "s..." ) end
            ...
            if casting and cast_time > 0 then
                ...
                if ability then
                    ...
                    destGUID  = Zekili:GetMacroCastTarget( ability.key, state.buff.casting.applied, "RESET" ) or state.target.unit
                end
                ...
                if dispName == "Primary" or dispName == "AOE" then
                    ...
                    if delay > 0 then
                        if Zekili.ActiveDebug then Zekili:Debug( "Advancing by %.2f per GCD or cast or channel or reset_postcast value.", delay ) end
                        state.advance( delay )
                    end
                end
        ...
        -- Zekili:Yield( "Reset Post-Casting" )
...
Zekili:ProfileCPU( "state.reset", state.reset )
...
function state:StartCombat()
    if self.time == 0 then
        ...
        if Zekili.ActiveDebug then Zekili:Debug( format( "Starting combat at %.2f -- time is %.2f.", self.false_start, self.time ) ) end
    end
...
function state.advance( time )
    ...
    while( event ) do
        if event.time <= state.query_time + time then
            ...
            if Zekili.ActiveDebug then Zekili:Debug( "While advancing by %.2f to %.2f, %s %s occurred at %.2f.", time, realOffset + time, event.action, event.type, state.offset ) end
...
do
    ...
    function Zekili:PlaceHold( action, combat, verbose )
        if not action then return end
        ...
        if ability then
            ...
            if verbose then Zekili:Print( class.abilities[ ability.key ].name .. " placed on hold" .. ( combat and " until end of combat." or "." ) ) end
            Zekili:ForceUpdate( "ZEKILI_HOLD_APPLIED" )
        end
    end

    function Zekili:RemoveHold( action, verbose )
        if not action then return end
        ...
        if ability and state.holds[ ability.key ] then
            ...
            if verbose then Zekili:Print( class.abilities[ ability.key ].name .. " hold removed." ) end
            Zekili:ForceUpdate( "ZEKILI_HOLD_REMOVED" )
        end
    end

    function Zekili:ToggleHold( action, combat, verbose )
    ...
    function Zekili:ReleaseHolds( combat )
        ...
        if holdRemoved then Zekili:ForceUpdate( "ZEKILI_COMBAT_HOLD_REMOVED" ) end
    end
end
...
function state:TimeToReady( action, pool )
    ...
    if ability.timeToReadyOverride then
        ...
        if override ~= nil then
            ...
            if Zekili.ActiveDebug then
                Zekili:Debug( "%s has a timeToReadyOverride. Returning %.2f.", action, override )
            end
    ...
    if z and z > wait then
        if Zekili.ActiveDebug then Zekili:Debug( "ICD is " .. z .. ", last cast was " .. lastCast .. ", remaining CD: " .. max( 0, lastCast + z - now ) ) end
        wait = z
    end
...
function state:IsReady( action )
    ...
    if not ability then
        Zekili:Error( "Failed state:IsReady( " .. ( action or "BLANK" ) .. " )." )
        return false
    end
...
function state:ClashOffset( action )
    ...
    local spec = rawget( Zekili.DB.profile.specs, state.spec.id )
