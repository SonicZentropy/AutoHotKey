...
ns.Error = function( output, ... )
    ...
    if not errors[ output ] then
        ...
        -- if Zekili.DB.profile.Verbose then Zekili:Print( output ) end
    else
...
function Zekili:Error( ... )
    ns.Error( ... )
end

Zekili.ErrorKeys = eIndex
Zekili.ErrorDB = errors


function Zekili:GetErrors()

    for i = 1, #eIndex do
        Zekili:Print( eIndex[i] .. " (n = " .. errors[ eIndex[i] ].n .. "), last at " .. errors[ eIndex[i] ].last .. "." )
    end
...
function Zekili:After( time, func, ... )
    local args = { ... }
...
function Zekili:GetSpellLinkWithTexture( id, size, color )
...
function Zekili:ZoomedTextureWithText( texture, text )
...
do
    local CurrentBuild = Zekili.CurrentBuild
    ...
    local FindStringInTooltip = function( str, id, ttType, reverse, useMatch )
        ...
        elseif ttType == "conduit" then data = C_TooltipInfo.GetConduit( id, 0 )
            ...
            Zekili:Error( "Usage:  FindStringInTooltip( str, id, { spell | item | inventory | conduit }, reverse, useMatch )\n" ..
    ...
    Zekili.FindStringInTooltip = FindStringInTooltip
    Zekili.FindStringInSpellTooltip = FindStringInSpellTooltip
    Zekili.FindStringInItemTooltip = FindStringInItemTooltip
    Zekili.FindStringInInventoryItemTooltip = FindStringInInventoryItemTooltip
    Zekili.FindStringInConduitTooltip = FindStringInConduitTooltip

    Zekili.IsSpellDisabled = IsSpellDisabled
    Zekili.IsItemDisabled = IsItemDisabled
    Zekili.IsInventoryItemDisabled = IsInventoryItemDisabled
...
do
    ...
    function Zekili:Loadstring( str )
...
do
    ...
    function ns.ClearMarks( super )
        ...
        if Zekili.ActiveDebug then Zekili:Debug( "Purged %d marked values in %.2fms.", count, endTime - startTime ) end
    end
