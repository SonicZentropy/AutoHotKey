...
local spec = Zekili:NewSpecialization( 270 )
...
spec:RegisterSetting( "save_faeline", false, {
    type = "toggle",
    name = strformat( "%s: Prevent Overlap", Zekili:GetSpellLinkWithTexture( spec.talents.jadefire_stomp[2] ) ),
    desc = strformat( "If checked, %s will not be recommended when %s and/or %s are active.\n\n"
        .. "Disabling this option may impact your mana efficiency.", Zekili:GetSpellLinkWithTexture( spec.talents.jadefire_stomp[2] ),
        Zekili:GetSpellLinkWithTexture( spec.auras.awakened_jadefire.id ), Zekili:GetSpellLinkWithTexture( spec.auras.jadefire_teachings.id ) ),
    width = "full",
} )

--[[ spec:RegisterSetting( "roll_movement", 5, {
    type = "range",
    name = strformat( "%s: Check Distance", Zekili:GetSpellLinkWithTexture( 109132 ), Zekili:GetSpellLinkWithTexture( 115008 ) ),
    desc = strformat( "If set above zero, %s (and %s) may be recommended when your target is at least this far away.", Zekili:GetSpellLinkWithTexture( 109132 ),
        Zekili:GetSpellLinkWithTexture( 115008 ) ),
    min = 0,
    max = 100,
    step = 1,
    width = "full"
...
spec:RegisterSetting( "aoe_rsk", false, {
    type = "toggle",
    name = function ()
        return strformat( "%s: AOE", Zekili:GetSpellLinkWithTexture( state.talent.rushing_wind_kick.enabled and spec.abilities.rushing_wind_kick.id or spec.abilities.rising_sun_kick.id ) )
    end,
    desc = function ()
        ...
        Zekili:GetSpellLinkWithTexture( state.talent.rushing_wind_kick.enabled and spec.abilities.rushing_wind_kick.id or spec.abilities.rising_sun_kick.id ), Zekili:GetSpellLinkWithTexture( spec.abilities.enveloping_mist.id ) )
    end,
    width = "full",
} )
...
spec:RegisterSetting( "single_zen_pulse", false, {
    type = "toggle",
    name = strformat( "%s (%s): Single Target", Zekili:GetSpellLinkWithTexture( spec.abilities.vivify.id ), Zekili:GetSpellLinkWithTexture( spec.auras.zen_pulse.id ) ),
    desc = strformat( "If checked, %s may be recommended with %s when there is only one enemy detected.\n\n",
        Zekili:GetSpellLinkWithTexture( spec.abilities.vivify.id ), spec.auras.zen_pulse.name ),
    width = "full",
} )
