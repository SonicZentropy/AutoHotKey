...
local spec = Zekili:NewSpecialization( 70 )
        ...
        elseif spellID == 408385 then
            ...
            if now - last_crusading_strike > 0.5 then -- Crusader Strikes: Swing Damage
                ...
                if current_crusading_strikes < 2 then
                    Zekili:ForceUpdate( "CRUSADING_STRIKES", true )
                end
            end
        end
    end
...
local CrusadingStrikes = setfenv( function()
    if not action.rebuke.in_range then
        if Zekili.ActiveDebug then Zekili:Debug( "Crusading Strikes energize fails: Out of range." ) end
        return
    end
...
local csStartCombat = setfenv( function()
    ...
    if not action.rebuke.in_range then
        if Zekili.ActiveDebug then Zekili:Debug( "Unable to forecast Crusading Strikes; out of range." ) end
        return
    end
    ...
    if current_crusading_strikes < 2 then
        ...
        if Zekili.ActiveDebug then Zekili:Debug( "First Crusading Strikes resource gain forecasted for next swing." ) end
        ...
        if Zekili.ActiveDebug then Zekili:Debug( "Immediate Crusading Strikes resource gain on virtual combat start." ) end
        spec.abilities.crusader_strike.handler()
    end
    ...
    if IsSpellKnownOrOverridesKnown( 427453 ) then
        if talent.lights_deliverance.enabled then
            ...
            if free_hol_triggered > 0 and action.hammer_of_light.lastCast > action.wake_of_ashes.lastCast then
                ...
                if Zekili.ActiveDebug then Zekili:Debug( "Hammer of Light active; applied hammer_of_light_free: %.2f : %.2f : %.2f : %d", buff.hammer_of_light_free.remains, free_hol_triggered, query_time, ld_stacks ) end
            else
                if Zekili.ActiveDebug then Zekili:Debug( "Hammer of Light active; hammer_of_light_free ruled out: %.2f : %.2f : %d", free_hol_triggered, query_time, ld_stacks ) end
            end
        end
        ...
        if not buff.hammer_of_light_free.up then
            ...
            if Zekili.ActiveDebug then Zekili:Debug( "Hammer of Light not active; applied hammer_of_light_ready: %.2f", buff.hammer_of_light_ready.remains ) end
        end

        if buff.hammer_of_light_ready.down and buff.hammer_of_light_free.down then
            if Zekili.ActiveDebug then Zekili:Debug( "Hammer of Light appears active [ %.2f ] but I don't know why; applying hammer_of_light_ready." ) end
    ...
    if time > 0 and talent.crusading_strikes.enabled then
        if not action.rebuke.in_range then
            ...
            if last_crusading_strike == 0 or now - last_crusading_strike > mh_speed then
                if Zekili.ActiveDebug then Zekili:Debug( "Unable to forecast Crusading Strikes swing; no prior swings have been detected or the last swing was more than 1 swing timer ago." ) end
            else
                ...
                if Zekili.ActiveDebug then
                    if last_crusading_strike == 0 then Zekili:Debug( "No prior Crusading Strikes swings have been detected; assuming first swing is non-productive." )
                    else Zekili:Debug( "Last Crusading Strikes swing was %.2f seconds ago (vs. %.2f swing timer); it was %s.", time_since, mh_speed, was_productive and "productive" or "non-productive" ) end
                end
