...
local spec = Zekili:NewSpecialization( 63 )
...
spec:RegisterCombatLogEvent( function( _, subtype, _,  sourceGUID, sourceName, _, _, destGUID, destName, destFlags, _, spellID, spellName )
    if sourceGUID == state.GUID and subtype == "SPELL_AURA_APPLIED" and ( spellID == spec.auras.heating_up.id or spellID == spec.auras.hot_streak.id ) then
        Zekili:ForceUpdate( spellName, true )
    end
end )
...
spec:RegisterStateFunction( "hot_streak", function( willCrit )
    ...
    if Zekili.ActiveDebug then Zekili:Debug( "*** HOT STREAK (Cast/Impact) ***\n    Heating Up: %s, %.2f\n    Hot Streak: %s, %.2f\n    Crit: %s, %.2f", buff.heating_up.up and "Yes" or "No", buff.heating_up.remains, buff.hot_streak.up and "Yes" or "No", buff.hot_streak.remains, willCrit and "Yes" or "No", stat.crit ) end
    ...
    if willCrit then
        ...
        if Zekili.ActiveDebug then Zekili:Debug( "*** HOT STREAK END ***\nHeating Up: %s, %.2f\nHot Streak: %s, %.2f", buff.heating_up.up and "Yes" or "No", buff.heating_up.remains, buff.hot_streak.up and "Yes" or "No", buff.hot_streak.remains ) end
        return true
    end
    ...
    if buff.heating_up.up then
        if query_time - buff.heating_up.applied > 0.2 then
            if Zekili.ActiveDebug then Zekili:Debug( "May not crit; Heating Up was applied %.2f ago, so removing Heating Up..", query_time - buff.heating_up.applied ) end
            removeBuff( "heating_up" )
        else
            if Zekili.ActiveDebug then Zekili:Debug( "May not crit; Heating Up was applied %.2f ago, so ignoring the non-crit impact.", query_time - buff.heating_up.applied ) end
        end
    end
...
Zekili:EmbedDisciplinaryCommand( spec )
    ...
    reset = setfenv( function ()
        if talent.incanters_flow.enabled then
            if not incanters_flow.fRegistered then
                Zekili:ProfileFrame( "Incanters_Flow_Arcane", incanters_flow.f )
                -- One-time setup.
        ...
        handler = function ()
            ...
            if hardcast or cast_time > 0 then
                ...
                if buff.sun_kings_blessing_ready.up then
                    ...
                    if Zekili.ActiveDebug then Zekili:Debug( "Applied Combustion." ) end
...
spec:RegisterSetting( "pyroblast_pull", false, {
    name = strformat( "%s: Non-Instant Opener", Zekili:GetSpellLinkWithTexture( spec.abilities.pyroblast.id ) ),
    desc = strformat( "If checked, a non-instant %s may be recommended as an opener against bosses.", Zekili:GetSpellLinkWithTexture( spec.abilities.pyroblast.id ) ),
    type = "toggle",
    width = "full"
} )

spec:RegisterSetting( "prevent_hardcasts", false, {
    name = strformat( "%s and %s: Instant-Only When Moving", Zekili:GetSpellLinkWithTexture( spec.abilities.pyroblast.id ), Zekili:GetSpellLinkWithTexture( spec.abilities.fireball.id ) ),
    desc = function()
        return strformat( "If checked, non-instant %s and %s casts will not be recommended while you are moving.\n\nAn exception is made if %s is talented and active and your cast " ..
                          ...
                          Zekili:GetSpellLinkWithTexture( spec.abilities.pyroblast.id ),
                          Zekili:GetSpellLinkWithTexture( spec.abilities.fireball.id ),
                          Zekili:GetSpellLinkWithTexture( class.auras.ice_floes.id ),
                          Zekili:GetSpellLinkWithTexture( class.auras.ice_floes.id ) )
    end,
    type = "toggle",
    width = "full"
