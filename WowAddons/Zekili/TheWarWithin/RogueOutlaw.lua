...
spec:RegisterCombatLogEvent( function( _, subtype, _,  sourceGUID, sourceName, _, _, destGUID, destName, destFlags, _, spellID, spellName )
    ...
    if spellID == 315508 then
        ...
        if Zekili.ActiveDebug then
            Zekili:Debug( "Updated lastRoll to %.2f, rollDuration to %.2f", lastRoll, rollDuration )
        end
    end
end )
...
spec:RegisterHook( "reset_precast", function()
    ...
    if Zekili.ActiveDebug and buff.roll_the_bones.up then
        ...
        Zekili:Debug( " - lastRoll: %.2f", lastRoll )
        Zekili:Debug( " - rollDuration: %.2f", rollDuration )
        Zekili:Debug( " - rtb_primary_remains: %.2f", rtb_primary_remains )

        Zekili:Debug( " - Buff Status (vs. %.2f):", rollDuration )
        for i = 1, 6 do
            local bone = rtb_buff_list[ i ]
            if buff[ bone ].up then
                local bone_duration = buff[ bone ].duration
                Zekili:Debug( "   * %-20s %5.2f : %5.2f %s",
                    bone,
                    buff[ bone ].remains,
                    bone_duration,
   ...
   --         if buff[ bone ].up then
   --             local bone_duration = buff[ bone ].duration
   --             Zekili:Debug( " - %-20s %5.2f : %5.2f %s", bone, buff[ bone ].remains, bone_duration, bone_duration < rollDuration and "shorter" or bone_duration > rollDuration and "longer" or "normal" )
   --         end
   --     end
   -- end
end )
    ...
    keep_it_rolling = {
        ...
        handler = function ()
           for _, v in pairs( rtb_buff_list ) do
                ...
                -- Optional Debugging
                if Zekili.ActiveDebug then
                    Zekili:Debug( "Keep It Rolling applied to '%s': New expires = %.2f (capped at 60 seconds).", v, buff[ v ].expires )
                end
            end
        end
    end,
