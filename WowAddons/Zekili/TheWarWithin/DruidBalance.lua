...
local spec = Zekili:NewSpecialization( 102 )
...
do
    ...
    function Zekili:EmbedAdaptiveSwarm( s )
...
local ExpireCelestialAlignment = setfenv( function()
    ...
    if Zekili.ActiveDebug then Zekili:Debug( "Expire CA_Inc: %s - Starfire(%d), Wrath(%d), Solar(%.2f), Lunar(%.2f)", eclipse.state, eclipse.starfire_counter, eclipse.wrath_counter, buff.eclipse_solar.remains, buff.eclipse_lunar.remains ) end
end, state )
...
local ExpireEclipseLunar = setfenv( function()
    ...
    if Zekili.ActiveDebug then Zekili:Debug( "Expire Lunar: %s - Starfire(%d), Wrath(%d), Solar(%.2f), Lunar(%.2f)", eclipse.state, eclipse.starfire_counter, eclipse.wrath_counter, buff.eclipse_solar.remains, buff.eclipse_lunar.remains ) end
end, state )
...
local ExpireEclipseSolar = setfenv( function()
    ...
    if Zekili.ActiveDebug then Zekili:Debug( "Expire Solar: %s - Starfire(%d), Wrath(%d), Solar(%.2f), Lunar(%.2f)", eclipse.state, eclipse.starfire_counter, eclipse.wrath_counter, buff.eclipse_solar.remains, buff.eclipse_lunar.remains ) end
end, state )
...
spec:RegisterStateTable( "eclipse", setmetatable( {
    ...
    reset = setfenv( function()
        ...
        elseif buff.eclipse_lunar.up then
            ...
            if eclipse.starfire_counter == 0 and eclipse.wrath_counter == 0 then
                if Zekili.ActiveDebug then Zekili:Debug( "Resetting from %d / %d to ANY_NEXT.", eclipse.starfire_counter, eclipse.wrath_counter ) end
                eclipse.reset_stacks()
            end
        end
    ...
    advance = setfenv( function()
        if Zekili.ActiveDebug then Zekili:Debug( "Eclipse Advance (Pre): %s - Starfire(%d), Wrath(%d), Solar(%.2f), Lunar(%.2f)", eclipse.state, eclipse.starfire_counter, eclipse.wrath_counter, buff.eclipse_solar.remains, buff.eclipse_lunar.remains ) end
        ...
        if Zekili.ActiveDebug then Zekili:Debug( "Eclipse Advance (Post): %s - Starfire(%d), Wrath(%d), Solar(%.2f), Lunar(%.2f)", eclipse.state, eclipse.starfire_counter, eclipse.wrath_counter, buff.eclipse_solar.remains, buff.eclipse_lunar.remains ) end
...
spec:RegisterHook( "step", function()
    if Zekili.ActiveDebug then Zekili:Debug( "Eclipse State: %s, Wrath: %d, Starfire: %d; Lunar: %.2f, Solar: %.2f\n", eclipse.state or "NOT SET", eclipse.wrath_counter, eclipse.starfire_counter, buff.eclipse_lunar.remains, buff.eclipse_solar.remains ) end
end )
...
spec:RegisterSetting( "vigil_damage", 50, {
    name = strformat( "%s Damage Threshold", Zekili:GetSpellLinkWithTexture( spec.abilities.natures_vigil.id ) ),
...
spec:RegisterSetting( "starlord_cancel", false, {
    name = strformat( "%s |TInterface\\Addons\\Zekili\\Textures\\Cancel:0|t Cancel", Zekili:GetSpellLinkWithTexture( spec.auras.starlord.id ) ),
    desc = strformat( "If checked, canceling |TInterface\\Addons\\Zekili\\Textures\\Cancel:0|t your %s may be recommended.  Canceling it allows you to start building stacks via %s and %s at its full duration.\n\n"
        .. "You will likely want a |cFFFFD100/cancelaura %s|r macro to manage this during combat.", spec.auras.starlord.name, Zekili:GetSpellLinkWithTexture( spec.abilities.starsurge.id ),
        Zekili:GetSpellLinkWithTexture( spec.abilities.starfall.id ), spec.auras.starlord.name ),
    icon = 462651,
...
--[[ spec:RegisterSetting( "delay_berserking", false, {
    name = strformat( "Delay %s", Zekili:GetSpellLinkWithTexture( class.specs[ 0 ].auras.berserking.id ) ),
