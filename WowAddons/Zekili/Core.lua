...
local function EmbedBlizOptions()
    local panel = CreateFrame( "Frame", "ZekiliDummyPanel", UIParent )
    panel.name = "Zekili"

    local open = CreateFrame( "Button", "ZekiliOptionsButton", panel, "UIPanelButtonTemplate" )
    open:SetPoint( "CENTER", panel, "CENTER", 0, 0 )
    open:SetWidth( 250 )
    open:SetHeight( 25 )
    open:SetText( "Open Zekili Options Panel" )

    open:SetScript( "OnClick", function ()
        ns.StartConfiguration()
    end )

    Zekili:ProfileFrame( "OptionsEmbedFrame", open )
...
function Zekili:OnInitialize()
    self.DB = LibStub( "AceDB-3.0" ):New( "ZekiliDB", self:GetDefaults(), true )
    ...
    LDS:EnhanceDatabase( self.DB, "Zekili" )
    ...
    AceConfig:RegisterOptionsTable( "Zekili", self.Options )

    -- EmbedBlizOptions()

    self:RegisterChatCommand( "zekili", "CmdLine" )
    self:RegisterChatCommand( "hek", "CmdLine" )

    local LDB = LibStub( "LibDataBroker-1.1", true )
    local LDBIcon = LDB and LibStub( "LibDBIcon-1.0", true )

    Zekili_OnAddonCompartmentClick = function( btn, arg1, arg2, checked, button )
    ...
    Zekili_OnAddonCompartmentEnter = function( addonName, button )
        GameTooltip:SetOwner( AddonCompartmentFrame )
        GameTooltip:AddDoubleLine( "Zekili", GetDataText() )
    ...
    Zekili_OnAddonCompartmentLeave = function( addonName, button )
        GameTooltip:Hide()
    end

    AddonCompartmentFrame:RegisterAddon({
        text = "Zekili",
        icon = "Interface\\AddOns\\Zekili\\Textures\\LOGO-ORANGE.blp",
        registerForAnyClick = true,
        notCheckable = true,
        func = Zekili_OnAddonCompartmentClick,
        funcOnEnter = Zekili_OnAddonCompartmentEnter,
        funcOnLeave = Zekili_OnAddonCompartmentLeave
    } )

    if LDB then
        ns.UI.Minimap = ns.UI.Minimap or LDB:NewDataObject( "Zekili", {
            type = "data source",
            text = "Zekili",
            icon = "Interface\\ICONS\\spell_nature_bloodlust",
            OnClick = Zekili_OnAddonCompartmentClick,
            OnEnter = function( self )
                GameTooltip:SetOwner( self )
                GameTooltip:AddDoubleLine( "Zekili", ns.UI.Minimap.text )
                GameTooltip:AddLine( "|cFFFFFFFFLeft-click to make quick adjustments.|r" )
                GameTooltip:AddLine( "|cFFFFFFFFRight-click to open the options interface.|r" )
                GameTooltip:Show()
            end,
            OnLeave = Zekili_OnAddonCompartmentLeave
        } )
        ...
        if LDBIcon then
            LDBIcon:Register( "Zekili", ns.UI.Minimap, self.DB.profile.iconStore )
        end
    end
...
function Zekili:OnEnable()
    ...
    if self.BuiltFor > self.CurrentBuild then
        self:Notify( "|cFFFF0000WARNING|r: This version of Zekili is for a future version of WoW; you should reinstall for " .. self.GameBuild .. "." )
    end
end


Zekili:ProfileCPU( "StartEventHandler", ns.StartEventHandler )
Zekili:ProfileCPU( "BuildUI", Zekili.BuildUI )
Zekili:ProfileCPU( "SpecializationChanged", Zekili.SpecializationChanged )
Zekili:ProfileCPU( "OverrideBinds", Zekili.OverrideBinds )
Zekili:ProfileCPU( "TotalRefresh", Zekili.TotalRefresh )


function Zekili:OnDisable()
...
function Zekili:AddToStack( script, list, parent, run )
...
function Zekili:PopStack()
    ...
    if x.priorMin then
        if self.ActiveDebug then Zekili:Debug( "Resetting delayMin to %.2f from %.2f.", x.priorMin, state.delayMin ) end
    ...
    if x.run then
        ...
        if scripts:CheckScript( x.script ) then
            ...
            if script.Recheck then
                ...
                if Zekili.ActiveDebug then Zekili:Debug( " - blocking script did not immediately block; will attempt to tune it." ) end
                ...
                if firstFail and firstFail > 0 then
                    ...
                    state.delayMin = actualDelay + firstFail
                    if Zekili.ActiveDebug then Zekili:Debug( " - setting delayMin to " .. state.delayMin .. " based on recheck and brute force." ) end
                else
                    state.delayMin = x.priorMin
                    -- Leave it alone.
                    if Zekili.ActiveDebug then Zekili:Debug( " - leaving delayMin at " .. state.delayMin .. "." ) end
                end
            end
        end
...
function Zekili:CheckChannel( ability, prio )
...
do
    ...
    function Zekili:IsSpellKnown( spell )
...
local Timer = {
    ...
    Reset = function( self )
        if not Zekili.ActiveDebug then return end
...
do
    ...
    local function FirstYield( self, msg, time )
        if Zekili.PLAYER_ENTERING_WORLD and not Zekili.LoadingScripts then
            self.Yield = DoYield
        end
    end

    Zekili.Yield = FirstYield
end


local invalidActionWarnings = {}

function Zekili:GetPredictionFromAPL( dispName, packName, listName, slot, action, wait, depth, caller )
            ...
            if self:IsActionActive( packName, listName, actID ) then
                ...
                elseif rWait <= state.cooldown.global_cooldown.remains and not state.spec.can_dual_cast and ability.gcd ~= "off" then
                    ...
                    elseif action == "trinket1" then
                        if state.trinket.t1.usable and state.trinket.t1.ability and not Zekili:IsItemScripted( state.trinket.t1.ability, true ) then
                    ...
                    elseif action == "trinket2" then
                        if state.trinket.t2.usable and state.trinket.t2.ability and not Zekili:IsItemScripted( state.trinket.t2.ability, true ) then
                                ...
                                elseif action == "variable" then
                                    ...
                                    if state.args.cycle_targets == 1 and state.settings.cycle and state.active_enemies > 1 then
                                        ...
                                        if state.cycle_enemies == 1 then
                                            if debug then Zekili:Debug( "There is only 1 valid enemy for target cycling; canceling cycle." ) end
                                            state.ClearCycle()
                                        end
                                    else
                                        state.ClearCycle()
...
Zekili:ProfileCPU( "GetPredictionFromAPL", Zekili.GetPredictionFromAPL )


function Zekili:GetNextPrediction( dispName, packName, slot )
    local debug = self.ActiveDebug
...
Zekili:ProfileCPU( "GetNextPrediction", Zekili.GetNextPrediction )
...
function Zekili:GetDisplayByName( name )
...
local aoeDisplayRule = function( p )
    ...
    local mode = Zekili:GetToggleState( "mode" )

    if mode == "dual" then return true end
    if mode == "reactive" and ns.getNumberTargets() < ( spec.aoe or 3 ) then
        if ZekiliDisplayAOE.RecommendationsStr then
            ZekiliDisplayAOE.RecommendationsStr = nil
            ZekiliDisplayAOE.NewRecommendations = true
        end
        return false
    end
...
function Zekili.Update()
    if not Zekili:ScriptsLoaded() then
        Zekili:LoadScripts()
        return
    end

    if not Zekili:IsValidSpec() then
        return
    end
    ...
    local debug = Zekili.ActiveDebug

    Zekili:GetNumTargets( true )
        ...
        if UI.Active and UI.alpha > 0 and rule( profile ) then
            ...
            for i = 1, numRecs do
                ...
                if debug and not isMain and #events > 0 then
                    Zekili:Debug( 1, "Ignoring %d events for [ %s ] display.", #events, dispName )
                end

                if isMain and #events > 0 then
                    if debug then Zekili:Debug( 1, "There are %d queued events to review.", #events ) end
                    ...
                    while( event ) do
                        ...
                        if debug then
                            ...
                            Zekili:Debug( 1, "Resources: %s\n", resources )

                            if state.channeling then
                                Zekili:Debug( 1, "Currently channeling ( %s ) until ( %.2f ).\n", state.channel, state.channel_remains )
                            end
                        end
                        ...
                        if t < 0 then
                            ...
                            if debug then Zekili:Debug( 1, "Finishing queued event #%d ( %s of %s ) due at %.2f because the event should've already occurred.\n", n, event.type, event.action, t ) end
                            state:HandleEvent( event )
                            state.offset = state.offset + t
                            event = events[ 1 ]
                        --[[ elseif t < 0.2 then
                            if debug then Zekili:Debug( 1, "Finishing queued event #%d ( %s of %s ) due at %.2f because the event occurs w/in 0.2 seconds.\n", n, event.type, event.action, t ) end
                            state.advance( t )
                            ...
                            if channeling then
                                if debug then Zekili:Debug( "We are channeling, checking if we should break the channel..." ) end
                                shouldBreak = Zekili:CheckChannel( nil, 0 )
                            ...
                            if ( casting or ( channeling and not shouldBreak ) ) and state.spec.can_dual_cast then
                                ...
                                for spell in pairs( state.spec.dual_cast ) do
                                    if debug then Zekili:Debug( "CWC: %s | %s | %s | %s | %.2f | %s | %.2f | %.2f", spell, tostring( state:IsKnown( spell ) ), tostring( state:IsUsable( spell ) ), tostring( class.abilities[ spell ].dual_cast ), state:TimeToReady( spell ), tostring( state:TimeToReady( spell ) <= t ), state.offset, state.delay ) end
                                ...
                                if eventAbility and not eventAbility.tick then
                                    ...
                                    for i = 1, #events do
                                        ...
                                        if e.type ~= "CHANNEL_TICK" then
                                            ...
                                            if debug then Zekili:Debug( "As %s's channel has no tick function, we will check between now and %s's %s event in %.2f seconds.", event.action, overrideAction, overrideType, overrideTime ) end
                                            break
                                        end
                                    end
                                end
                            ...
                            if ( casting or channeling ) and not shouldBreak and not shouldCheck then
                                ...
                                if debug then Zekili:Debug( 1, "Queued event #%d (%s %s) due at %.2f; checking pre-event recommendations.\n", overrideIndex or n, overrideAction or event.action, overrideType or event.type, overrideTime or t ) end

                                if casting or channeling then
                                    state:ApplyCastingAuraFromQueue()
                                    if debug then Zekili:Debug( 2, "Player is casting for %.2f seconds.  %s.", state.buff.casting.remains, shouldBreak and "We can break the channel" or "Only spells castable while casting will be used" ) end
                                else
                                ...
                                repeat
                                    action, wait, depth = Zekili:GetNextPrediction( dispName, packName, slot )

                                    if action == "wait" then
                                        if debug then Zekili:Debug( "EXECUTING WAIT ( %.2f ) EVENT AT ( +%.2f ) AND RECHECKING RECOMMENDATIONS...", slot.waitSec, wait ) end
                                    ...
                                    if waitLoop > 2 then
                                        if debug then Zekili:Debug( "BREAKING WAIT LOOP!" ) end
                                        slot.action = nil
                                        slot.actionName = nil
                                        slot.actionID = nil
                                ...
                                if not action then
                                    if debug then Zekili:Debug( "Time spent on event #%d PREADVANCE: %.2fms...", n, debugprofilestop() - eStart ) end
                                    if debug then Zekili:Debug( 1, "No recommendation found before event #%d (%s %s) at %.2f; triggering event and continuing ( %.2f ).\n", n, event.action or "NO ACTION", event.type or "NO TYPE", t, state.offset + state.delay ) end

                                    state.advance( overrideTime or t )
                                    if debug then Zekili:Debug( "Time spent on event #%d POSTADVANCE: %.2fms...", n, debugprofilestop() - eStart ) end
                        ...
                        if n > 10 then
                            if debug then Zekili:Debug( "WARNING:  Attempted to process 10+ events; breaking to avoid CPU wastage." ) end
                            break
                        end

                        Zekili.ThreadStatus = "Processed event #" .. n .. " for " .. dispName .. "."
                    end
                end
                ...
                if not action then
                    ...
                    if debug then
                        ...
                        Zekili:Debug( 1, "Resources: %s", resources or "none" )
                        ns.callHook( "step" )

                        if state.channeling then
                            Zekili:Debug( " - Channeling ( %s ) until ( %.2f ).", state.channel, state.channel_remains )
                        end
                    end
                ...
                if debug then
                    Zekili:Debug( "Recommendation #%d is %s at %.2fs (%.2fs).", i, action or "NO ACTION", wait or state.delayMax, state.offset + state.delay )
                end
                ...
                if action then
                    ...
                    slot.keybind, slot.keybindFrom = Zekili:GetBindingForAction( action, display, i )
                    ...
                    if i < display.numIcons then
                        ...
                        if cast > 0 then
                            if not ability.channeled then
                                if debug then Zekili:Debug( "Queueing %s cast finish at %.2f [+%.2f] on %s.", action, state.query_time + cast, state.offset + cast, cast_target ) end
                                ...
                                if debug then Zekili:Debug( "Queueing %s channel finish at %.2f [%.2f+%.2f].", action, state.query_time + cast, state.offset, cast, cast_target ) end
                                ...
                                if ability.tick_time then
                                    ...
                                    if debug then Zekili:Debug( "Queued %d ticks of channel %s.", ticks, action ) end
                                end

                                if Zekili.Scripts.Channels and Zekili.Scripts.Channels[ packName ] and Zekili.Scripts.Channels[ packName ][ action ] then
            ...
            if WeakAuras and WeakAuras.ScanEvents then
                ...
                WeakAuras.ScanEvents( "ZEKILI_RECOMMENDATION_UPDATE", dispName, Queue[ 1 ].actionID, Queue[ 1 ].indicator, Queue[ 1 ].empower_to, UI.EventPayload )
            end
            ...
            if debug then
                ...
                if Zekili:SaveDebugSnapshot( dispName ) then
                    ...
                    if Zekili.Config then LibStub( "AceConfigDialog-3.0" ):SelectGroup( "Zekili", "snapshots" ) end
                end
            end

            if round < 5 then Zekili:Yield( "Recommendations finished for " .. dispName .. ".", nil, true ) end
    ...
    if snaps then
        Zekili:Print( "Snapshots saved:  " .. snaps .. "." )
    end
end
Zekili:ProfileCPU( "ThreadedUpdate", Zekili.Update )


function Zekili_GetRecommendedAbility( display, entry )
    entry = entry or 1

    if not rawget( Zekili.DB.profile.displays, display ) then
    ...
    local payload = Zekili.DisplayPool[ display ].EventPayload
