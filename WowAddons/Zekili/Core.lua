...
local aoeDisplayRule = function( p )
    ...
    if Zekili:GetToggleState( "mode" ) == "reactive" and ns.getNumberTargets() < ( spec.aoe or 3 ) then
        ...
        if UI.Active and UI.alpha > 0 and rule( profile ) then
            ...
            for i = 1, numRecs do
                ...
                if debug and #events > 0 then
                    Zekili:Debug( 1, "There are %d queued events to review.", #events )
                end
            ...
            if round < 5 then Zekili:Yield( "Recommendations finished for " .. lastDisplay .. ".", nil, true ) end
        else
