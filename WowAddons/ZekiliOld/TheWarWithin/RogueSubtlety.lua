...
local spec = Zekili:NewSpecialization( 261 )
...
spec:RegisterHook( "reset_precast", function( amt, resource )
    if talent.danse_macabre.enabled then
        ...
        if Zekili.ActiveDebug then
            Zekili:Debug( "Danse Tracker @ Reset:" )
            for k, v in pairs( danse_macabre_tracker ) do
                Zekili:Debug( "  " .. k .. " = " .. tostring( v ) )
            end
        end
    end
...
spec:RegisterHook( "step", function()
    if Zekili.ActiveDebug then
        Zekili:Debug( "Danse Tracker @ Step:" )
        for k, v in pairs( danse_macabre_tracker ) do
            Zekili:Debug( "  " .. k .. " = " .. tostring( v ) )
        end
    end
...
spec:RegisterSetting( "rupture_duration", 12, {
    name = strformat( "%s Duration", Zekili:GetSpellLinkWithTexture( 1943 ) ),
    desc = strformat( "If set above 0, %s will not be recommended if the target will die within the timeframe specified.\n\n"
        ...
        Zekili:GetSpellLinkWithTexture( 1943 ), class.specs[ 259 ].abilities.rupture.name ),
    type = "range",
    min = 0,
    max = 18,
    step = 0.1,
